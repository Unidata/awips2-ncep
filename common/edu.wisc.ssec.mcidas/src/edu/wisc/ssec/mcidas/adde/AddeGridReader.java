...
/**
 * AddeGridReader interface for McIDAS ADDE grid data sets. Simulates a McIDAS
 * GRDLIST request using an ADDE URL.
 *
 * <pre>
...
public class AddeGridReader {
     ...
     *            </pre>
     *
     * @exception AddeURLException
     *                if there are no datasets of the particular type or there
        ...
        try {
            ...
            while ((check = dis.readInt()) == 0) {
                ...
                while ((check2 = dis.readInt()) == 0) {
                    ...
                    // SAG McIDASGridDirectory mg = new
                    // McIDASGridDirectory(header);
    ...
    /**
     * creates an ArrayList of arrays of data, plus an ArrayList of grid headers
     * (McIDASGridDirectories) which are then available using the
     * getGridHeaders() method.
     *
     * @param request
     *            ADDE URL to read from. See class javadoc.
     *
        ...
        try {
            ...
            if (checkBytes != numBytes) {
                throw new AddeURLException(
                        "Invalid number of bytes returned for grid.");
            }
            ...
            for (int i = 0; i < numGrids; i++) {
                /*
                 * SAG dis.readFully(header,0,256); String name = new
                 * String(header,24,4); McIDASGridDirectory mg = new
                 * McIDASGridDirectory(header);
                 * System.out.println(mg.toString()); //SAG CoordinateSystem c =
                 * mg.getCoordinateSystem(); gridHeaders.add(mg); int rows =
                 * mg.getRows(); int cols = mg.getColumns();
                 * //System.out.println("# rows & cols = "+rows+" "+cols);
                 *
                 * double scale = mg.getParamScale();
                 * //System.out.println("param scale = "+scale+" gridType="+mg.
                 * getGridType());
                 *
                 * double[] ddata = new double[rows*cols]; int n = 0; // store
                 * such that 0,0 is in lower left corner... for (int nc=0;
                 * nc<cols; nc++) { for (int nr=0; nr<rows; nr++) { int temp =
                 * dis.readInt(); ddata[(rows-nr-1)*cols + nc] = // check for
                 * missing value (temp == McIDASUtil.MCMISSING) ? Double.NaN : (
                 * (double) temp) / scale ; } } gridData.add(ddata); SAG
                 */
                check = dis.readInt();
                // System.out.println("check after grid = "+check+" point value
                // = "+data[100]);
                if (check != 0) {
                    break;
                }
...
//                  ? "adde://adde.unidata.ucar.edu/griddirectory?group=rtgrids&descr=ngm&num=all&proj=6999&user=tomw&"
        ...
        /*
         * ArrayList v = d.getGridDirectory(request); ArrayList vh =
         * d.getFileHeaders(); String s = (String) vh.elementAt(0);
         * System.out.println("File header = "+s);
         *
         * for (int i=0; i<v.size(); i++) { McIDASGridDirectory mg =
         * (McIDASGridDirectory) v.elementAt(i);
         * System.out.println(mg.toString()); }
         */
