 ...
 * ------------- -------- --------- -------------------------------------
 * Jul 29, 2020  8204     randerso  Organize imports and code formatting
 *
 * </pre>
 *
 * @author tomw
...
public class AddeServerInfo extends Object {
    ...
    /**
     * Creates new AddeServerInfo. This collects information about ADDE
     * server(s) -- their groups, datasets, and date-times (right now only image
     * date-time implemented). This is a helper class for anything else that
     * needs to get these information.
     *
     * The candidate list of ADDE servers should be obtained from a
     * yet-to-be-identified "well-known list", and made available in this class.
     */
     ...
     *
     * @param l
     *            a list of ADDE servers that can be used in lieu of
    ...
    public AddeServerInfo(String[] l) {
        ...
        String[] list = l;
        if (list == null) {
            list = new String[] { "adde.unidata.ucar.edu",
                    "suomi.ssec.wisc.edu", "motherlode.ucar.edu",
    ...
    /**
     * get name of the server that has been selected and for which all group and
     * dataset info is currently valid for
     *
    ...
    /**
     * define which server to select, and the type of ADDE data group (image,
     * point, grid, text) to get group and descriptor info for
     *
     * This is the workhorse of the code.
     *
     * @param s
     *            the name of the ADDE server to be selected
     * @param type
    ...
    public int setSelectedServer(String s, String type) {
        ...
        try {
            ...
            for (int i = 0; i < table.size(); i++) {
                ...
                if (debug) {
                    System.out.println("indexType = " + indexType + " dataType="
                            + dataType + " indexN1,N2,C=" + indexN1 + " "
                            + indexN2 + " " + indexC);
                }
                ...
                if (!val[indexType].equalsIgnoreCase(dataType)) {
                    continue;
                }
                ...
                for (int j = 0; j < groups.size(); j++) {
                    ...
                    // element: 0=groupname, 1=Vector of datasets, 2=Vector of
                    // descrs
            ...
            if (debug) {
                System.out
                        .println("Status from RTFBand=" + rtfBand.getStatus());
            }
            ...
            if (rtfBand.getStatus().equals("OK")) {
                ...
                for (int i = 0; i < num; i++) {
                    ...
                    if (debug) {
                        System.out.println("band = " + bandNames[i]);
                    }
                }
                hasBandNames = true;
            ...
            status = "ADDE group & dataset information retrieved from server "
                    + s + ".";
            istatus = 0;
    ...
    /**
     * get a list of all datasets valid for this server and the designated
     * group.
     *
    ...
    public String[] getDatasetList() {
        ...
        status = "Dataset list for server " + selectedServer + " and group "
                + selectedGroup + " not found.";
        return null;
    }

    /**
     * get a list of all dataset descriptions for this server and the designated
     * group
     *
    ...
    /**
     * get the valid date-time list for the selected server/group/dataset. Note
     * that you must 'setSelectedGroup()' and 'setSelectedDataset()' prior to
     * using this method.
     *
     * [Also, Don Murray at Unidata wrote the original - thanks!]
     *
     * @return list of date-times ("yy-MM-dd / hh:mm:ss" format) or the string
     *         "No data available"
     *
     *
     */
    public String[] getDateTimeList() {
        ...
        if (!hasGroup || !hasDataset) {
            return null;
        }
        StringBuffer addeCmdBuff = new StringBuffer("adde://" + selectedServer
        ...
        try {
            AreaDirectoryList adir = new AreaDirectoryList(
                    addeCmdBuff.toString());
            ...
            for (int i = 0; i < numTimes; i++) {
                times[i] = sdf.format(dirs[i][0].getNominalTime(),
                        new StringBuffer(), new FieldPosition(0)).toString();
            }
            dirsTimes = times;
        } catch (McIDASException e) {
            status = "Error getting times";
        }
        return times;
    }
     ...
     *
     * @param d
     *            the name of the dataset/descr to select
     */
    ...
    */
    ...
    /**
     * set the userID and proj number if required for transactions, as required
     * by servers that use accounting.
     *
     * @param up
     *            the user/project number request string in the form
     *            "user=tom&proj=1234"
     *
     *
     */
            ...
            for (int k = 0; k < b.length; k++) {
                ...
                if (i == 0 && k == 0) {
                    ...
                    for (String element : dt) {
                        System.out.println("DateTime = " + element);
                    }
                }
            }
