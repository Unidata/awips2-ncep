...
public class NsharpLibXwvid {
    ...
    public static EffectiveLayerPressures effective_inflow_layer(
            List<NcSoundingLayer> sndLys, float ecape, float ecinh,
            Parcel muParcel)
    /******************************************************************/
    ...
    /******************************************************************/
    {
        ...
        LParcelValues lparcelVs = NsharpLibSkparams.define_parcel(sndLys,
                NsharpLibSndglib.PARCELTYPE_MOST_UNSTABLE, 300);
        Parcel parcel = NsharpLibSkparams.parcel(sndLys, -1.0F, -1.0F,
        ...
        if (mucape >= 100 && mucin >= -250) {
            ...
            for (int i = sfcIndex; i <= soundingSize - 1; i++) {
                ...
                parcel = NsharpLibSkparams.parcel(sndLys, -1.0F, -1.0F,
                        sndLayer.getPressure(), sndLayer.getTemperature(),
                        sndLayer.getDewpoint());

                if ((parcel.getBplus() >= ecape)
                        && (parcel.getBminus() >= ecinh)) {
            ...
            for (int i = sfcIndex; i <= soundingSize - 1; i++) {
                ...
                if (sndLayer.getPressure() <= effLayerParams.getBottomPress()) {
                    if (NsharpLibBasics.qc(sndLayer.getTemperature())
                            && NsharpLibBasics.qc(sndLayer.getDewpoint())) {

                        parcel = NsharpLibSkparams.parcel(sndLys, -1.0F, -1.0F,
                                sndLayer.getPressure(),
                                sndLayer.getTemperature(),
                                sndLayer.getDewpoint());
                        if ((parcel.getBplus() <= ecape)
                                || (parcel.getBminus() <= ecinh))
                        /*
                         ...
                         */
                        {
                            ...
                            while (ok) {
                                ...
                                if (NsharpLibBasics.qc(sndLayer2
                                        .getTemperature())
                                        && NsharpLibBasics.qc(sndLayer2
                                                .getDewpoint())) {
    ...
    public static float damaging_wind(List<NcSoundingLayer> sndLys,
            Parcel mmParcel)
    /********************************************************************/
...

    {
        ...
        lapseRate = NsharpLibSkparams.lapse_rate(sndLys,
                sfcLayer.getPressure(),
        ...
        WindComponent windComp = NsharpLibWinds.mean_wind_npw(
                sndLys,
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 1000.0f)),
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 3500.0f)), 0);
    ...
    /********************************************************************/
    {
        ...
        NcSoundingLayer sfcLayer = NsharpLibBasics.sfc(sndLys);
        lapseRate = NsharpLibSkparams
                .lapse_rate(sndLys, sfcLayer.getPressure(), NsharpLibBasics
                        .i_pres(sndLys, sfcLayer.getGeoHeight() + 3000f));
        if (lapseRate < 7.0) {
            lapseRate = 7.0f;
        }
    ...
    public static float scp(List<NcSoundingLayer> sndLys, float stdir,
            float stspd, Parcel muParcel)
    /***************************************************************/
    /*                                                             */
    ...
    /***************************************************************/
    {
        if (!NsharpLibBasics.qc(stdir) || !NsharpLibBasics.qc(stspd)
                || muParcel == null) {
            return NsharpLibSndglib.NSHARP_NATIVE_INVALID_DATA;
        }
        // original code calls define_parcel(3, 400); and then calls parcel()
        ...
        float el = NsharpLibBasics.agl(sndLys,
                NsharpLibBasics.i_hght(sndLys, muParcel.getElpres()));

        EffectiveLayerPressures effLyPress = effective_inflow_layer(sndLys,
                100, -250, muParcel);
        float base = NsharpLibBasics.agl(sndLys,
                NsharpLibBasics.i_hght(sndLys, effLyPress.getBottomPress()));
        float depth = (el - base);
        ...
        WindComponent shearWindComp = NsharpLibWinds.wind_shear(
                ...
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, base + (depth * 0.5f))));
        float eshear = shearWindComp.getWspd();

        if ((cape >= 100) && (el < 0)) {
            shearWindComp = NsharpLibWinds.wind_shear(
                    sndLys,
                    NsharpLibBasics.i_pres(sndLys,
                            NsharpLibBasics.msl(sndLys, 0)),
                    NsharpLibBasics.i_pres(sndLys,
                            NsharpLibBasics.msl(sndLys, 6000f)));
            eshear = shearWindComp.getWspd();
        }
        Helicity helicity = NsharpLibWinds.helicity(sndLys, base,
                NsharpLibBasics.agl(sndLys, NsharpLibBasics.i_hght(sndLys,
                        effLyPress.getTopPress())), stdir, stspd);
    ...
    public static WindComponent[] bunkers_storm_motion(
            List<NcSoundingLayer> sndLys, Parcel muParcel)
    /*************************************************************
     ...
     * supercell
     * 
     * @param : Parcel - most unstable parcel
     ...
     *         left motion wind component
     * Updated based on bigSharp9.3 2017Feb27 version - Chin 06/30/2017 
     *************************************************************/
    {
        ...
        if (mucp >= 100 && mucn >= -250 && el > 0 && depth >= 3000) {
            /* 21 March 2014 - include Bunkers et al. (2014) findings using 65% of depth from inflow base to mu parcel EL height */      
            WindComponent meanWndComp = NsharpLibWinds.mean_wind(
                    sndLys,
                    effLyPress.getBottomPress(),
                    NsharpLibBasics.i_pres(
                            sndLys,
                            NsharpLibBasics.msl(sndLys, base
                                    + (depth * 0.65f))));
            
            WindComponent wndShearComp = NsharpLibWinds
                    .wind_shear(
                            sndLys,
                            effLyPress.getBottomPress(),
                            NsharpLibBasics.i_pres(
            ...
            if (NsharpLibBasics.qc(wndShearComp.getUcomp())
                    && NsharpLibBasics.qc(wndShearComp.getVcomp())) {
                ucompRight = meanWndComp.getUcomp()
                        + ((d / (float) Math
                                .pow((wndShearComp.getUcomp() * wndShearComp
                                        .getUcomp())
                                        + (wndShearComp.getVcomp() * wndShearComp
                                                .getVcomp()), 0.5f)) * wndShearComp
                                .getVcomp());
             ...
             */
            /* Sfc-6km mean wind */
            WindComponent meanWndComp = NsharpLibWinds.mean_wind_npw(
                    sndLys,
                    NsharpLibBasics.sfcPressure(sndLys),
                    NsharpLibBasics.i_pres(sndLys,
                            NsharpLibBasics.msl(sndLys, 6000.0f)), 0);
            /* Sfc-6km Shear Vector */
            WindComponent wndShearComp = NsharpLibWinds.wind_shear(
                    sndLys,
                    NsharpLibBasics.sfcPressure(sndLys),
                    NsharpLibBasics.i_pres(sndLys,
                            NsharpLibBasics.msl(sndLys, 6000.0f)));
            if (NsharpLibBasics.qc(wndShearComp.getUcomp())
                    && NsharpLibBasics.qc(wndShearComp.getVcomp())) {
                ucompRight = meanWndComp.getUcomp()
                        + ((d / (float) Math
    ...
    public static float sigtorn_cin(List<NcSoundingLayer> sndLys, float stdir,
            float stspd, Parcel muParcel, Parcel mmParcel)
    /*****************************************************************/
    ...
    /*****************************************************************/
    {
        ...
        float lclh = NsharpLibBasics.agl(sndLys,
                NsharpLibBasics.i_hght(sndLys, mmParcel.getLclpres()));
        ...
        float stp_cin = (eshear) * (esrh / 150) * (mlcape / 1500) * (lclh)
                * (mlcinh);

        if (NsharpLibBasics.agl(sndLys,
                NsharpLibBasics.i_hght(sndLys, effLyPress.getBottomPress())) > 0.0) {
            stp_cin = 0;
        }
        if (stp_cin < 0) {
            stp_cin = 0;
    ...
    public static float sigtorn_test(List<NcSoundingLayer> sndLys, float stdir,
            float stspd, Parcel muParcel, Parcel mmParcel)
    /*****************************************************************/
    /* Significant Tornado Parameter (effective layer with CIN) */
    /* Input: stdir and stspd are the storm wind direction and */
    ...
    public static float sigtorn_fixed(List<NcSoundingLayer> sndLys,
            float stdir, float stspd, Parcel sfcParcel)
    /***************************************************************/
    /*                                                             */
    ...
    /***************************************************************/
    {
        ...
        float lclh = NsharpLibBasics.agl(sndLys,
                NsharpLibBasics.i_hght(sndLys, sfcParcel.getLclpres()));
        ...
        Helicity helicity = NsharpLibWinds.helicity(sndLys, 0, 1000, stdir,
                stspd);
    ...
    public static float sig_hail(List<NcSoundingLayer> sndLys, float lr75,
            float t500, float shear6k, float fzlh, Parcel mlParcel,
            Parcel muParcel)
    /***************************************************************
    ...
    public static float sigtorn_lr(List<NcSoundingLayer> sndLys, float stdir,
            float stspd, Parcel muParcel, Parcel mmParcel, EffectiveLayerPressures effPress)
    /***************************************************************/
    /*                                                             */
    ...
    /***************************************************************/
    {
        float ix1, ix2, ix3, ix4, eshear, esrh, mlcape, lclh, stp_lr, lpl, el, depth;
        float pres, oldlplpres, base, cape;
        int zIndex;
        short oldlplchoice;
        ...
        if ((cape >= 100) && (el < 0)) {
            windS = NsharpLibWinds.wind_shear(sndLys,
                    NsharpLibBasics.i_pres(sndLys,NsharpLibBasics.msl(sndLys,0)),
                    NsharpLibBasics.i_pres(sndLys,NsharpLibBasics.msl(sndLys,6000)));
            eshear = windS.getWspd();
        }

        Helicity hel = NsharpLibWinds.helicity(sndLys,base, NsharpLibBasics.agl(sndLys,NsharpLibBasics.i_hght(sndLys,effPress.getTopPress())), stdir, stspd);
        ...
        float lr = NsharpLibSkparams.lapse_rate(sndLys,NsharpLibBasics.sfcPressure(sndLys), NsharpLibBasics.i_pres(sndLys,NsharpLibBasics.sfcHeight(sndLys)+500));
    ...
    public static int ww_type(List<NcSoundingLayer> sndLys, Parcel mlParcel,
            Parcel muParcel, Parcel sbParcel, float stdir, float stspd)
    /********************************************************************
     ...
     ********************************************************************/
    {
        if (mlParcel == null || muParcel == null || sbParcel == null
                || !NsharpLibBasics.qc(stdir) || !NsharpLibBasics.qc(stspd)) {
            return 0;
        }
        /* sb parcel */
        ...
        /* sighail ingredients */
        float lapseRate700To500 = NsharpLibSkparams
                .lapse_rate(sndLys, 700, 500);
        WindComponent windComp6k = NsharpLibWinds.wind_shear(
                sndLys,
                surfacePressure,
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 6000)));
        float shr6kSpd = windComp6k.getWspd();

        WindComponent windComp8k = NsharpLibWinds.wind_shear(
                sndLys,
                surfacePressure,
                NsharpLibBasics.i_pres(sndLys,
        ...
        float fzlh = NsharpLibBasics.mtof(NsharpLibBasics.agl(
                sndLys,
                NsharpLibBasics.i_hght(sndLys,
                        NsharpLibSkparams.temp_lvl(sndLys, 0))));
        float sighail = sig_hail(sndLys, lapseRate700To500, temp500m,
                NsharpLibBasics.kt_to_mps(shr6kSpd), fzlh, mlParcel, muParcel);

        float rm_scp = scp(sndLys, stdir, stspd, muParcel);
        float wind_dmg = damaging_wind(sndLys, mlParcel);

        WindComponent srwindComp4To6k = NsharpLibWinds.sr_wind(
                sndLys,
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 4000)),
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 6000)), stdir, stspd);

        float sr4kTo6kSpd = srwindComp4To6k.getWspd();
        float srhelicity1k = NsharpLibWinds.helicity(sndLys, 0, 1000, stdir,
                stspd).getTotalHelicity();
        EffectiveLayerPressures effLyPress = NsharpLibXwvid
                .effective_inflow_layer(sndLys, 100, -250, muParcel);
        float bot = NsharpLibBasics.agl(sndLys,
                NsharpLibBasics.i_hght(sndLys, effLyPress.getBottomPress()));
        float top = NsharpLibBasics.agl(sndLys,
                NsharpLibBasics.i_hght(sndLys, effLyPress.getTopPress()));
        float esrhelicity = NsharpLibWinds.helicity(sndLys, bot, top, stdir,
                stspd).getTotalHelicity();

        float lpaseRate1k = NsharpLibSkparams.lapse_rate(sndLys,
                surfacePressure, NsharpLibBasics.i_pres(sndLys, NsharpLibBasics
                        .sfc(sndLys).getGeoHeight() + 1000));

        float midrh = NsharpLibSkparams.mean_relhum(sndLys,
                surfacePressure - 150, surfacePressure - 350);
        float lowrh = NsharpLibSkparams.mean_relhum(sndLys, -1.0F,
                surfacePressure - 150);
        ...
        int ww_choice;
        if ((sig_tor >= 3.0) && (sig_tor_winter >= 4.0)
                && (srhelicity1k >= 200) && (esrhelicity >= 200)
                && (sr4kTo6kSpd >= 15.0) && (shr8kSpd >= 45.0)
                && (sblcl < 1000) && (mllcl < 1200) && (lpaseRate1k >= 5.0)
                && (mlcn > -50.0) && (bot == 0.0)) {
            ww_choice = 5;
        } else if (((sig_tor >= 3.0) || (sig_tor_winter >= 4.0))
                && (mlcn > -125.0) && (bot == 0.0)) {
            ww_choice = 4;
        } else if (((sig_tor >= 1.0) || (sig_tor_winter >= 1.0))
                && ((sr4kTo6kSpd >= 15.0) || (shr8kSpd >= 40.0))
                && (mlcn > -75.0) && (bot == 0.0)) {
            ww_choice = 4;
        } else if (((sig_tor >= 1.0) || (sig_tor_winter >= 1.0))
                && (low_mid_rh >= 60) && (lpaseRate1k >= 5.0) && (mlcn > -50.0)
                && (bot == 0.0)) {
            ww_choice = 4;
        } else if (((sig_tor >= 1.0) || (sig_tor_winter >= 1.0))
                && (mlcn > -150.0) && (bot == 0.0)) {
            ww_choice = 3;
                ...
                && (mlcn > -50.0) && (bot == 0.0)) {
            ww_choice = 3;
        } else if (((sig_tor_winter >= 1.0) || (rm_scp >= 4.0) || (sig_tor >= 1.0))
                && (mucn >= -50.0)) {
            ww_choice = 2;
        } else if ((rm_scp >= 2.0) && ((sighail >= 1.0) || (dncp >= 750))
                && (mucn >= -50.0)) {
            ww_choice = 2;
        } else if ((cbsig >= 30000) && (mmp >= 0.6) && (mucn >= -50.0)) {
            ww_choice = 2;
        } else if ((mucn >= -75.0)
                && ((wind_dmg >= 0.5) || (sighail >= 0.5) || (rm_scp >= 0.5))) {
            ww_choice = 1;
        } else {
            ww_choice = 0;
        }
    ...
    /*************************************************************
    /*  Convert from 
    /*  LAPSE_RATE_MAX of xwvid1.c           
    /*  original author: Rich Thompson SPC OUN                                  
    /*                                                          
    /*  Calculates the max lapse rate (C/km) in the specified    
    /*  layer depth, from the layer bottom and top               
    /*  Value is returned both as (param) and as a RETURN.       
    /*                               
    /*    bot = bottom of sounding layer (m AGL)                
    /*    top = top of sounding layer (m AGL)            
    /*    depth = depth of lapse rate calculation (m)       
    /*    Lapse rate returned in C/km    
   ...
   /*************************************************************/
    {
        ...
        /* loop through layers every 250 m, beginning at bot AGL , up to "depth" layer*/
        for (float z=bot; z<=(top-depth); z+=250) {
            float lr = NsharpLibSkparams.lapse_rate(sndLys, NsharpLibBasics.i_pres(sndLys,NsharpLibBasics.msl(sndLys,z)), 
    ...
    /*    Developed by Johnson and Sugden (2014)
    /* 
    /*  Ported from bigSharp9.3-2017feb27 xwvid2.c
    /*  Input; current storm direction and speed  
    ...
    /********************************************************************/
    {
        ...
        WindComponent shearWindComp = NsharpLibWinds.wind_shear(
                sndLys, NsharpLibBasics.sfcPressure(sndLys), NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 6000.0f)));

        shr6 = shearWindComp.getWspd() *.514f;

        lr75 = NsharpLibSkparams.lapse_rate(
                sndLys, 700.0F, 500.0F);


        /* DIR difference between mean EL to EL-1.5km wind and mean 3-6 km wind */
        /* no pressure weighting in mean wind calculations */
        WindComponent meanWndComp36 = NsharpLibWinds.mean_wind_npw(
                sndLys,
                NsharpLibBasics.i_pres(sndLys,
        ...
        WindComponent meanWndCompEl = NsharpLibWinds.mean_wind_npw(
                sndLys,
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, el-1500.0f)),
                elPress, 0);


        grwel = meanWndCompEl.getWdir() - meanWndComp36.getWdir(); //direl - dir36;
        if (grwel >= 180.0) {
            grwel = -10.0f;
        }

        /* DIR difference between 3-6 km and 0-1 km SR winds */
        WindComponent wndStmCompSfc = NsharpLibWinds.sr_wind(
                sndLys,
                NsharpLibBasics.sfcPressure(sndLys),
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 1000.0f)), st_dir, st_spd);
        WindComponent wndStmComp36 = NsharpLibWinds.sr_wind(
                sndLys,
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 3000.0f)),
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, 6000.0f)), st_dir, st_spd);
        
        sr_diff = wndStmComp36.getWdir() - wndStmCompSfc.getWdir(); 

        /* BWD between surface wind and mean wind EL to EL-750m */
        WindComponent wndShearElComp = NsharpLibWinds.wind_shear(
                sndLys,
                NsharpLibBasics.sfcPressure(sndLys),
                NsharpLibBasics.i_pres(sndLys,
                        NsharpLibBasics.msl(sndLys, el-750))); 
...

    {
        ...
        WindComponent windShearBP = NsharpLibWinds.wind_shear(sndLys,effLyPress.getBottomPress(), NsharpLibBasics.i_pres(sndLys,NsharpLibBasics.msl(sndLys,base + (depth * 0.5f))));
        ebwd = (((windShearBP.getWspd() *.514f) - 8)/10);
        if (ebwd < 0) {
            ebwd = 0;
        }

        float lapsedRate = NsharpLibSkparams.lapse_rate(sndLys, NsharpLibBasics.sfcPressure(sndLys), NsharpLibBasics.i_pres(sndLys,NsharpLibBasics.sfcHeight(sndLys)+3000));
        if (lapsedRate < 4) {
            lapsedRate = 4;
        }
        lllr = (float) (Math.pow((lapsedRate - 4), 2)/4);

        WindComponent windShearSfc1500 = NsharpLibWinds.wind_shear(sndLys, NsharpLibBasics.sfcPressure(sndLys),NsharpLibBasics.i_pres(sndLys,NsharpLibBasics.sfcHeight(sndLys)+1500));
