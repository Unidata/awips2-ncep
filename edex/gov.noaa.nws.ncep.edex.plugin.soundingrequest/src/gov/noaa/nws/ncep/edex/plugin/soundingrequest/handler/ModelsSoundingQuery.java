...
public class ModelsSoundingQuery {
    ...
    private static List<NcSoundingProfile> getMdlSndInterpolatedDataProfileList(
        ...
        if (levels.size() == 0) {
            logger.info("getModelLevels return 0;  file=" + refTime
                    + " stime=" + validTime 
                    + " modeltype=" + mdlName);
    ...
    public static NcSoundingModel getMdls() {
        ...
        Object[] mdlName = (Object[]) dao.executeSQLQuery(queryStr);
    ...
    private static List<NcSoundingLayer> getModelSfcLayers(
        ...
        try {
            ...
            // combine the two lists of FHAG records to read from the datastore only once
            ...
            /* Validate that each surface level has a pressure level or geoheight.*/
            for (int i = 0; i < points.size(); i++) {
     ...
     *
     *         Created @ 3/28/2012
     * #6674 update to support interpolation 
     */
    ...
    private static List<NcSoundingProfile> queryNearestPtProfileListByPointGroup(
             ...
             List<?> levels,Coordinate[]  latLonArray) {
        List<NcSoundingProfile> soundingProfileList = new ArrayList<NcSoundingProfile>();
                ...
                for (int k = 0; k < latLonArray.length; k++) {
                    ...
                    if (pt == null) {
                        logger.info("getLatLonNearestPt return 0; lat=" + lat
                                + " lon=" + lon + " stime=" + validTime +  " modeltype=" + modelName);
                    } else {
                        points.add(pt);
                    }
                }
    ...
    /**
     * Interpolate the surrounding Sounding Profiles at a level into the single Sounding Profile at the requested point.
     *  
     * @param soundingProfileList The list of Sounding Profiles at the same level surrounding the point of the 
     * @param xfrac
     * @param yfrac
     * @return
     */
    ...
    /**
     * Handles a grid model data request by creating and returning the desired Sounding Cube.
     * 
     * @param request
     *            The request for grid model data.
     * @return The sounding cube created for the request.
     */
        ...
        if (request.getLatLonAry() != null && refTimeStrArr!=null
            ...
            for(String validTimeStartStr : rangeTimeStrArr){
                if(request.isInterpolation() == true){
                    ...
                    soundingProfileList = getMdlSndInterpolatedDataProfileList(request.getLatLonAry(),
                            refTime, validTimeStartStr,request.getModelType());
                }
                else {
                    // get nearest point data
                    soundingProfileList = getMdlSndNearestPtDataProfileList(request.getLatLonAry(),
                            refTime, validTimeStartStr,request.getModelType());
                }
