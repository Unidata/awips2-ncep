 ...
 * ($GEMTBL/nmap/mod_res.tbl, with possible field customizations) into
 * corresponding files in the new system.
 * 
 * This is one of several programs in a suite intended to permit one-time
 * conversion of legacy Stored Procedure Files (SPFs) and all of their
 ...
 * 21 Oct 2013              B. Hebbard  Add GDATTIM and TEXT parameters (as optional)
 * </pre>
 * 
 * @author B. Hebbard
 * @version 1
...
public class ModresConvert {
    ...
    // [NO LONGER USED] private final static String
    // attrSetGroupListOutputFileName = "ModelFcstGridContours.xml";
    ...
    private enum GempakParameterPriority {
        // TODO: Need a default-value-required...?
    ...
    private enum GempakParametersToTransfer {
        ...
        // -- be transferred forward to the new attribute file (along with
        // whatever value was specified in the legacy file, even if null).
        // Parameters not listed here will be excluded from the transfer;
        // that is, they will not appear at all in the generated attribute file.
        //
        // If a parameter in this list does not appear in the legacy file at
        // all, the "priority" parameter determines whether it will be inserted
        // in the generated attribute file: If REQUIRED, it will be forced into
        // the new file, along with a default value as specified here.
        // If OPTIONAL, it will not appear in the converted attribute file
        // if not in the legacy one.

        // @formatter:off
        GDATTIM (GempakParameterPriority.OPTIONAL, ""),
        ...
        TEXT    (GempakParameterPriority.OPTIONAL, ""),
        ...
        // Constructor
        GempakParametersToTransfer(GempakParameterPriority priority,
                String defaultValue) {
    ...
    static {
        // Dumb. Just maps a string back to its enum value. Why? valueOf()
        // conversion throws an exception if string isn't in the enum,
        // and we want to reserve exceptions for exceptional conditions.
        // Here we might have this happen a lot -- a string from legacy
        // GEMPAK that's not in the list of things we want to carry forward.
        // So here we can use a keySet().contains() type of thing...
        for (GempakParametersToTransfer param : GempakParametersToTransfer
                .values()) {
    ...
    public boolean ingestModres(String modresTable) {
        System.out
                .println("\n[Info:  Reading legacy model product selection table "
                        + modresTable + "...]");
        ...
        try {
            ...
            while ((t = br.readLine()) != null) {
                ...
                if (!(t.startsWith("!")) && !t.isEmpty()) {
                    ...
                    if (result.length == 3) {
                        System.out.println("[Warning:  Line '" + t
                                + "' missing model name(s); ignored]");
                    } else if (result.length < 3) {
                        System.out.println("[Warning:  Line '" + t
                                + "' has too few fields; ignored]");
                    } else {
                        if (result.length > 4) {
                            System.out
                                    .println("[Info:  Line '"
                                            + t
                                            + "' has more than 4 fields; combining 4+ into single model field]");
                        ...
                        String attrFilename = createAttrFilename(product,
                                filepath);
                        // TODO: Explain how things are inside-out in the new
                        // system...
                        for (String model : models) {
                            // since attrSetGroupMap is a SortedMap, contents
                            // will be ordered by model/group in output
                            String key = model + " " + group;
                            ...
                            // or maybe filename??
                            // asg.addAttrSetName(product.toLowerCase());
                            // can't use this any more; new version checks for
                            // file existence, but we haven't created it yet!!
                            // asg.addAttrSetName(attrFilename);

                            // GH - only made compile. Not tested.
                            // was... , new File("bogusFile"));
                            // TODO: Better way??
                            asg.addAttrSetName(attrFilename);
                            // attrSetGroupMap.put(key, asg);
                        }
                        if (new File(filepath).exists()) {
                            restoreFileToAttrFileMap
                                    .put(filepath, attrFilename);
                        }
                    }
                    // lineCount++;
                }
    ...
    private String createAttrFilename(String product, String filepath) {
        ...
        String attrFilename = product + "_"
                + legacyRestoreFile.getParentFile().getName();

        if (!legacyRestoreFile.exists()) {
            // TODO: Consider whether we should abandon this entry in the XML
            // file too
            System.out.println("[Warning:  Restore File " + filepath
    ...
    /**
     * convert the modres file from .tbl to .xml (and maybe legacy restore files
     * to new attribute files)
     * 
    ...
    private void convertAllRestoreFilesToAttrFiles(String outDir) {
        ...
        for (String restoreFile : restoreFileToAttrFileMap.keySet()) {
            ...
            convertOneRestoreFileToAttrFile(new File(restoreFile), new File(
                    outDir + "/" + attrFile + ".attr"));
        }
    ...
    private void convertOneRestoreFileToAttrFile(File restoreFile, File attrFile) {
        ...
        System.out.println("\n[Info:  Converting legacy restore file "
                + restoreFile + " ...]");
        System.out
                .println("[       ...to new attribute file " + attrFile + "]");
        ...
        // Create a set to remember the parameters (of those that we're
        // interested in) that we've seen coming from the restore file

        EnumSet<GempakParametersToTransfer> parametersInLegacyRestoreFile = EnumSet
                .noneOf(GempakParametersToTransfer.class);
        ...
        try {
            ...
            while ((t = br.readLine()) != null) {
                t = t.trim();
                // Comments and blank lines get moved verbatim to the new
                // attribute file
                if (t.startsWith("!") || t == "\n") {
                    bw.write(t + "\n");
                } else {
                    ...
                    if (gempakParametersMap.containsKey(result[0])) {
                        ...
                        if (result.length > 1) {
                            ...
                            // String cdr =
                            // t.substring(result[0].length()).trim();
                            String cdr = t.substring(8);
                            bw.write(result[0]);
                            for (int i = result[0].length(); i < 6; i++)
                                bw.write(" "); // align
                            bw.write(" = " + cdr + "\n");
                        } else {
                            // ...without a value -- get the default one...?
                            // (or not?)
                            bw.write(result[0]);
                            for (int i = result[0].length(); i < 6; i++)
                                bw.write(" "); // align
                            bw.write(" = "
                                    + gempakParametersMap.get(result[0])
                                            .getDefaultValue() + "\n");
                        }
                        // either way, remember that this parameter has been
                        // done
                        parametersInLegacyRestoreFile.add(gempakParametersMap
                                .get(result[0]));
                    }
                }
            }
            // Check for parameters on our 'known' list that did not appear in
            // the legacy file.
            // For each, if 'REQUIRED', add it to the output anyway, with its
            // default value.
            boolean parametersAddedByConverter = false;
            for (GempakParametersToTransfer p : EnumSet
                    .complementOf(parametersInLegacyRestoreFile)) {
                if (p.getPriority() == GempakParameterPriority.REQUIRED) {
                    ...
                    bw.write(p.toString());
                    for (int i = p.toString().length(); i < 6; i++)
                        bw.write(" ");
                    bw.write(" = " + p.getDefaultValue() + "\n");
                } else {
                    System.out.println("[Info:  Optional parameter " + p
                            + " not in legacy, so not in new either]");
                }
            }
            bw.flush();
        } catch (IOException e) {
    ...
    private void writeAttrSetGroupsAsXMLFiles(String outDir) {
        for (AttrSetGroup asg : attrSetGroupMap.values()) {
            // Collections.sort(asg.getAttrSetNames()); // if want 'em in
            // order...?
            String outFile = outDir + "/" + asg.getResource() + "-"
                    + asg.getAttrSetGroupName() + ".xml";
            try {
                System.out
                        .println("\n[Info:  Writing attribute set group as XML file "
                                + outFile + "...]");
                // SerializationUtil.jaxbMarshalToXmlFile(colorMap, outFile);
                JAXBContext context = JAXBContext
                        .newInstance(AttrSetGroupList.class);
                Marshaller marshaller = context.createMarshaller();
                marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
                marshaller.marshal(asg, new FileWriter(new File(outFile)));
            } catch (JAXBException e) {
                // TODO: Maybe not this specific condition; other things can
                // trip exception
                System.out
                        .println("[Error:  The attrSet XML file is not writable.]");
                return;
            } catch (IOException e) {
                System.out
                        .println("[Error:  The attrSet XML file is not writable.]");
            ...
            // e.printStackTrace();
            // }
            System.out.println("[Info:  ..." + outFile
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 0
                && (args[0].equals("--help") || args[0].equalsIgnoreCase("/h"))) {
        ...
        while (!haveValidInput) {
            ...
            while (inputFileStr == null) {
                System.out.println("Enter legacy table (default "
                        + defaultInputFile + "):  ");
        ...
        while (!haveValidOutput) {
            ...
            while (outputDirStr == null) {
                System.out.println("Enter output directory (default "
                        + defaultOutputDir + "):  ");
                BufferedReader in = new BufferedReader(new InputStreamReader(
                        System.in));
                try {
