...
/**
 * HILO contains the information for plotting relative highs and lows in the
 * following format:
 * 
 * colorh;colorl/symbolh;symboll/rangeh;rangel/radius/counth;countl/interp
 * 
 * Colorh and colorl are the colors for the high and low symbols to be plotted.
 * If only a single number is entered, it will be used for both highs and lows.
 * The default for this entry is 0.
 * 
 * Symbolh and symboll specify the symbols to be plotted. The format for the
 * symbol input is: character # precision
 * 
 * where the character is the character to be plotted. If the character is an
 * integer, markers corresponding to that number will be plotted. If the
 * character is an integer preceeded by the character 'S', special symbols
 * corresponding to that number will be plotted. Information about markers can
 * be found in the help for MARKER. The # is a flag to plot values beneath the
 * marker. The integer following the # is the number of decimal places to
 * display in the value. If a # is present without the following number, integer
 * values are printed. The default for the symbols is H;L.
 * 
 * Rangeh and rangel are ranges for highs and lows specified as: minval - maxval
 * 
 * where minval and maxval are integers which specify the range of values to be
 * considered for designation as a high or low. The default is to consider all
 * data.
 * 
 * The search radius is a sqaure region of grid points. The region is a moving
 * search area in which all points are compared to derive relative extrema. The
 * default is 3, which is generally the most effective. To find more
 * concentrated areas of highs and lows, use a smaller radius. A large radius,
 * such as 10 or higher, is not very effective.
 * 
 * Counth and countl are integer values for the maximum number of high and low
 * values to be displayed. The default is 20;20.
 * 
 ...
 * 
 * In general, the above defaults are used if there is no entry for a part. For
 * the parts which have values for both relative highs and lows, a single entry
 * will be used for both highs and lows.
 * <p>
 * 
 * <pre>
 * SOFTWARE HISTORY
...
public class HILOStringParser {
    /**
     * The object that actually validates each parsed input and generates the
     * lists of attributes for the HiLo parameters.
     **/
    ...
    protected enum HILOParseErrCodes {
        EMPTY(0), INVALID_COLOR(1), COLOR_OOR(2), INVALID_COLOR_HI(3), COLOR_HI_OOR(
                4), INVALID_COLOR_LO(5), COLOR_LO_OOR(6), INVALID_SYMBOL_PREC_HI(
                7), INVALID_SYMBOL_PREC_LO(8), INVALID_RANGE_HI_MIN(9), INVALID_RANGE_HI_MAX(
                10), RANGE_HI_MIN_GT_MAX(11), INVALID_RANGE_LO_MIN(12), INVALID_RANGE_LO_MAX(
                13), RANGE_LO_MIN_GT_MAX(14), INVALID_RADIUS(15), INVALID_COUNT_HI(
                16), INVALID_COUNT_LO(17);
    ...
    /*
     * Parse HILO input string This subroutine scans the user input for HILO.
     * This consists of six input strings separated by slashes. When a string
     * can be two substrings separated by a semicolon, the first substring
     * applies to highs, the second to lows. If no substring is given a default
     * is provided. The table below summarizes the input format and gives the
     * defaults.
     * 
     * STRING Description Format Example Default 1 Color number i;i 1 0 2
     * Symbol#precision c#i;c#i H#2;L#1 H;L 3 Range i-i;i-i 30-40 0-0;0-0 4
     * Search radius i 4 3 5 Count i;i 10;5 20;20 6 Interpolation flag l T F
     */
    private boolean parseHILOString(String hilo) {

        boolean parseCorrected = true;
        ...
        while ((pos = hiloStr.indexOf("/", start)) >= 0) {
            if (pos != start) {
                ...
                if (cnt == 0) {
                    ...
                    parseCorrected = parseColorHiLo(tmp);
                    if (!parseCorrected)
                        return parseCorrected;
                } else if (cnt == 1) {
                    // parse symbol high/low
                    parseCorrected = parseSymbolHiLo(tmp);
                    if (!parseCorrected)
                        return parseCorrected;
                } else if (cnt == 2) {
                    // parse range high/low
                    parseCorrected = parseRangeHiLo(tmp);
                    if (!parseCorrected)
                        return parseCorrected;
                } else if (cnt == 3) {
                    // parse radius
                    parseCorrected = parseRadius(tmp);
                    if (!parseCorrected)
                        return parseCorrected;
                } else if (cnt == 4) {
                    // parse count high/low
                    parseCorrected = parseConutHiLo(tmp);
                    if (!parseCorrected)
                        return parseCorrected;
                } else if (cnt == 5) {
                    // parse interpolation flag
    ...
    private boolean parseColorHiLo(String colorHiLoStr) {
        ...
        if (colorHiLoStr.contains(";")) {
                ...
                while ((pos = colorStr.indexOf(";", start)) >= 0) {
                    if (pos != start) {
                        ...
                        if (tmp.matches(hiloExpre)) {
                            ...
                            if ((value > 0) && (value <= 32)) {
                                if (cnt == 0) {
                                    this.hiloBuilder.setColorHi(GempakColor
                                            .convertToRGB(value));
                                    this.hiloBuilder.setInputColorHi(value);
                                } else {
                                    this.hiloBuilder.setColorLo(GempakColor
                                            .convertToRGB(value));
            ...
            if (colorHiLoStr.matches(hiloExpre)) {
                ...
                if ((value > 0) && (value <= 32)) {
                    this.hiloBuilder
                            .setColorHi(GempakColor.convertToRGB(value));
                    this.hiloBuilder
                            .setColorLo(GempakColor.convertToRGB(value));
                    this.hiloBuilder.setInputColorHi(value);
                    this.hiloBuilder.setInputColorLo(value);
                } else {
                    hiloParsed = false;
    ...
    /*
     * parse symbol high/low If the first string is a single number, that number
     * is the color number for both highs and lows. In the second string, the c
     * in the table stands for either a character string, or a marker or special
     * symbol number. Special symbols are specified by preceeding the number
     * with an 'S'. The # sign is a flag to plot the values beneath the marker.
     * The number following # is the number of decimal places to display in the
     * value. If the number is absent, integer values are displayed.
     * 
     * The defaults above apply if no string is entered. If one of the entries
     * of a string is entered, it is assumed to apply to highs unless it is
     * preceded by a semicolon. The omitted entry defaults according to the
     * table below:
     * 
     * STRING DEFAULT FOR OMITTED PART 1 Same value 2 No symbol 3 Missing values
     * 5 Same value
     */
    ...
    private boolean parseSymbol(String symbol, int cnt) {
        ...
        if ((pos = symbol.indexOf("#", 0)) >= 0) {
            ...
            if (cnt == 0)
                this.hiloBuilder.setSymbolHiPlotValue(true);
            else
                this.hiloBuilder.setSymbolLoPlotValue(true);
        } else {
            tmp1 = symbol;
        }
        ...
        if (tmp1 != null) {
            ...
            if (tmp1.matches(hiloExpre)) {
                if (cnt == 0) {
                    ...
                    this.hiloBuilder.setSymbolLoMarkerNumber(Integer
                            .valueOf(tmp1));
                    this.hiloBuilder.setSymbolLoType(2);
                }
            } else if (tmp.matches(hiloExpre)) {
                // check marker number
                if (cnt == 0) {
                    this.hiloBuilder.setSymbolHi(" ");
                    this.hiloBuilder.setSymbolHiMarkerNumber(Integer
                ...
                // check the first character for 'S'
                if (firstChar.toUpperCase().compareTo("S") == 0) {
                    if (cnt == 0)
                        this.hiloBuilder.setSymbolHiType(3);
                    else
                        this.hiloBuilder.setSymbolLoType(3);
                } else {
                    if (cnt == 0)
                        this.hiloBuilder.setSymbolHiType(2);
                    else
                ...
                // the string are characters before '#' sign
                if (cnt == 0) {
        ...
        // parse precision
        if (tmp2 != null) {
            // check and see precision is positive integer
            if (tmp2.matches(hiloExpre) && (Integer.valueOf(tmp2) >= 0)) {
                if (cnt == 0)
                    this.hiloBuilder.setSymbolHiNumOfDecPls(Integer
                            .valueOf(tmp2));
                else
                    this.hiloBuilder.setSymbolLoNumOfDecPls(Integer
                            .valueOf(tmp2));
            } else {
                if (cnt == 0)
                    this.setErrorMessage(getParaserErrorDec(HILOParseErrCodes.INVALID_SYMBOL_PREC_HI.value));
                else
                    this.setErrorMessage(getParaserErrorDec(HILOParseErrCodes.INVALID_SYMBOL_PREC_LO.value));
                parse = false;
            }
        }
        return parse;
    ...
    /*
     * Rangeh and rangel are ranges for highs and lows specified as: minval -
     * maxval where minval and maxval are integers which specify the range of
     * values to be considered for designation as a high or low. The default is
    ...
    private boolean parseRange(String rangeStr, int cnt) {
        ...
        if (tmp1 != null) {
            if (tmp1.matches(hiloExpre)) {
                ...
                if (cnt == 0)
                    this.hiloBuilder.setRangeHiMinval(minVal);
                else
                    this.hiloBuilder.setRangeLoMinval(minVal);
            } else {
                if (cnt == 0) {
        ...
        if (parse && (tmp2 != null)) {
            if (tmp2.matches(hiloExpre)) {
                ...
                if (cnt == 0)
                    this.hiloBuilder.setRangeHiMaxval(maxVal);
                else
                    this.hiloBuilder.setRangeLoMaxval(maxVal);
            } else {
                if (cnt == 0) {
        ...
        while ((pos = countStr.indexOf(";", start)) >= 0) {
            if (pos != start) {
                ...
                if (tmp.matches(hiloExpre) && (Integer.valueOf(tmp) >= 0)) {
                    if (cnt == 0)
                        this.hiloBuilder.setCountHi(Integer.valueOf(tmp));
                    else
                        this.hiloBuilder.setCountLo(Integer.valueOf(tmp));
                } else {
                    if (cnt == 0) {
     ...
     * tokens using a blank-space as a delimiter. Then it concatenates the
     * tokens, as long as the tokens are non-empty.
     * 
     * @param String
     *            inputString
