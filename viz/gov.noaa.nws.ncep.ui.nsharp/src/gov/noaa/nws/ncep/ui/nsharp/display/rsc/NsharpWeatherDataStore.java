...
public class NsharpWeatherDataStore {
    ...
    public static final String[] STORM_MOTION_TYPE_STR = { "SFC-1km",
            ...
            "LCL-EL(Cloud Layer)", "Lower Half SR Depth" };
    ...
    // height in ft of Wet-bulb at freezing level
    ...
    // STP test
    private float stpTest = NsharpLibSndglib.NSHARP_NATIVE_INVALID_DATA;
    ...
    public void setStorm(float speed, float direction) {
        ...
        // compute STP-test
        if (muParcel == null || mlParcel == null) {
            stpTest = NsharpLibSndglib.NSHARP_NATIVE_INVALID_DATA;
        } else {
            stpTest = NsharpLibXwvid.sigtorn_test(soundingLys, smdir, smspd,
                    muParcel, mlParcel);
        }
    ...
    private void computeParcelParameters() {
        ...
        for (int parcelNumber = 1; parcelNumber <= NsharpLibSndglib.PARCEL_MAX; parcelNumber++) {
            ...
            if (parcelNumber >= NsharpLibSndglib.PARCELTYPE_OBS_SFC
                    && parcelNumber < NsharpLibSndglib.PARCELTYPE_MEAN_MIXING) {
