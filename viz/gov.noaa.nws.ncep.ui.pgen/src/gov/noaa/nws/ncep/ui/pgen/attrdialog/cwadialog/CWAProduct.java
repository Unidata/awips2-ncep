...
public class CWAProduct {
        ...
        for (String line : lines) {
            if (line.startsWith(CWSUid + " CWA")
                ...
                if (!expire_date.isEmpty()) {
                    ...
                    Calendar expCal = TimeUtil.newCalendar();
                    expCal.set(refTime.get(Calendar.YEAR),
                            refTime.get(Calendar.MONTH), expireDay, expireHour,
                            expireMinute);
    ...
    public int getNextSeriesId(boolean isCor) {
        ...
        if (!getSeries().trim().isEmpty()) {
            try {
                ...
                issuanceNum = issuanceNum % 99;
            } catch (NumberFormatException nfe) {
                logger.error("Failed to parse " + getSeries(), nfe);
            }
        }

        // the next issuance number if not isCor
        if (!isCor) {
            if (issuanceNum == 99) {
                issuanceNum = 1;
            } else {
    ...
    public boolean sendText(String site) {
        ...
        if (isOperational) {
            ...
            // productId is 9 characters. ModifyProduct:getProductHeader()
            // compares starts 4th character of AwipsWanPil
            // to the starts the 3rd character of afosid from afos2awips.txt.
            // But for CWA afosid is 9 characters long. For example
            // productId='SATCWAFW1' and afosid is also 'SATCWAFW1',
            // In ModifyProduct:getProductHeader(), it would compare 'WAFW1' to
            // 'CWAFW1'. That seems is a bug.
            // Anyway, add a space for setAwipsWanPil() to fix the issue.
            oup.setAwipsWanPil(" " + productId.trim());
            oup.setSource("CWSU " + site);
