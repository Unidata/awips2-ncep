    ...
    public static void main(String[] args) throws IOException {
        ...
        while (stdin.hasNext() && stdin.next().equalsIgnoreCase("Yes")) {
            ...
            while (stdin.hasNext() && stdin.next().equalsIgnoreCase("Yes")) {
                ...
                System.out
                        .println("Add a new Drawable for this Layer(Yes/No)?");

                while (stdin.hasNext() && stdin.next().equalsIgnoreCase("Yes")) {
                    oneLayer.addElement(generateOneDrawable());
                    System.out
                            .println("Add another Drawable for this Layer(Yes/No)?");
                }

                System.out
                        .println("Add another Layer for this Product(Yes/No)?");
            }

            productManager.add(oneProduct);

            System.out
                    .println("Do you want to generate another product (Yes/No)?");

        }

        System.out.println("\nTotal of " + productManager.size()
                + " product in PGEN:");
...
class AttributeGenerator implements IArc, IAvnText, ICombo, IKink,
        IMidCloudText, ISymbolSet, IText, ITrack, IVector, IWatchBox {
    ...
    private Color[] colorList = new Color[] { Color.red, Color.green,
            Color.blue };
    ...
    public AttributeGenerator(DrawableType deType) {
        ...
        if (deType == DrawableType.SYMBOL) {
            ...
            System.out
                    .println("Please input clear flag (0 = false\t1 = true):");
            ...
            System.out
                    .println("Please input closed flag (0 = false\t1 = true):");
            closed = (stdin.nextInt() == 0) ? false : true;

            System.out
                    .println("Please input filled flag (0 = false\t1 = true):");
            ...
            while (stdin.hasNextLine()) {
                if (stdin.hasNext()) {
                    ...
                    points.add(new Coordinate(Double.parseDouble(lat), Double
                            .parseDouble(lon)));
            ...
            lat = stdin.next();
            lon = stdin.next();
            points.add(new Coordinate(Double.parseDouble(lon), Double
                    .parseDouble(lat)));
            ...
            lpi = stdin.nextInt();
            arrowHeadType = (lpi == 0) ? (ArrowHeadType.FILLED)
                    : (ArrowHeadType.OPEN);
