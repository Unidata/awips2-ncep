 ...
 * "$GEMPAK/source/gemlib/cv/cvmdfy.c" with minimum changes to maintain the
 * rules in the original modification algorithm. The modification is done in the
 * "pixel" coordinate.
 * 
 * <pre>
 * SOFTWARE HISTORY
 ...
 * Jan 11, 2021    84064       smanoj      Fix Null Pointer Exception issues.
 * 
 * </pre>
 * 
 * @author J. Wu
...
public class PgenModifyLine {
     ...
     */
    // Original line
    private double[][] originalPts = null;

    // Clicked line
    private double[][] clickPts = null;

    // Closure of the original line
    private boolean closed = false;

    // Smooth level of the original line
     ...
     */
    // New line after applying modification
    private double[][] modifiedPts = null;

    // Line for ghosting after applying modification
    private double[][] ghostPts = null;

    // Starting point of ghost line on new line
    private int is = 0;

    // End point of ghost line on new line
    private int ie = 0;

    // Total number of point on new line
    private int np = 0;
     ...
     * Constructor
     */
    public PgenModifyLine(double[][] opt, double[][] cpt, boolean closed,
            int smthLevel) {
    ...
    public void PerformModify() {
        ...
        /*
         * Ensure the first point is the same as the last point for a closed
         * line.
         */
        double[][] origPts;
        if (closed) {
     ...
     ************************************************************************/
    {
        ...
        *------------------------------------------------------------------*/
        ...
        // false = not modified true - already modified
        modified = false;

        // index of FCP on OL if FCP=v
        op_index = -1;

        // index of FCP on OL if FCP online
        os_index = -1;
        ...
        if (op_index < 0 && os_index < 0) {
            ...
            if (closed && npc > 1 && (Math.abs(xc[0] - xc[npc - 1]) < PRECISION)
        ...
        if (!modified) {
            ...
            if (((op_index != -1 || os_index != -1) && (npc < 2))
                    || ((closed && (dfl <= TIE_DIST)) && (npc <= 3
                            || Math.abs(fang1 - fang2) < PRECISION))) {
        ...
        if (!modified && sm_lvl != 1 && sm_lvl != 2) {
            ...
            newLine = cvm_mdfy(sm_x, sm_y, xcp, ycp, closed, op_index,
                    os_index);

            cvm_index(newLine.length, is, ie,
                    closed); /* Indexing for ghost line */
        ...
        if (!modified) {
            ...
            double[][] tmpLine = cvm_mdfy(sm_x, sm_y, xcp, ycp, closed,
                    op_index, os_index);
            ...
            for (ii = 0; ii < tmpLine.length; ii++) {
                ...
                if (flag == -1) {
                    for (jj = 0; jj < npo; jj++) {
                        ...
                        if (flag == -1
                                && Math.abs(
                                        (xo[jj] - tmpLine[ii][0])) < PRECISION
                                && Math.abs((yo[jj]
                                        - tmpLine[ii][1])) < PRECISION) {
            ...
            if (closed) {
                ...
                if (opd >= 0) {
                    ...
                    for (ii = 0; ii < nn - 1; ii++) {
                        if (osd == -1 && ((Math
                                .abs(newLine[ii][0] - xc[npc - 1]) < PRECISION
                                && Math.abs(newLine[ii][1]
                                        - yc[npc - 1]) < PRECISION
                                && Math.abs(newLine[ii + 1][0]
                                        - xo[opd]) < PRECISION
                                && Math.abs(newLine[ii + 1][1]
                                        - yo[opd]) < PRECISION)
                                || (Math.abs(
                                        newLine[ii][0] - xo[opd]) < PRECISION
                                        && Math.abs(newLine[ii][1]
                                                - yo[opd]) < PRECISION
                                        && Math.abs(newLine[ii + 1][0]
                                                - xc[npc - 1]) < PRECISION
            ...
            for (ii = is + 1; ii < nn; ii++) {
                if (flag == -1
                        ...
                        && Math.abs(
                                (ycp[npc - 1] - newLine[ii][1])) < PRECISION) {
            ...
            //Indexing for ghost line
            cvm_index(np, is, ie, closed);
        }

        /*
         * chop off the last point from a closed line to return an open line,
         * the 'line_closed' attribute is still reserved via 'closed'.
         */
        double[][] modLine = null;
    ...
    private int cvm_cptv(double[] xin, double[] yin, double xci, double yci,
            boolean closed)
    /************************************************************************
     * cvm_cptv                                                             *
     *                                                                      *
     ...
     ***********************************************************************/
    {
        ...
        // index of LCP on OL if LCP=v
        epi = -1;

        // index of LCP on OL if LCP online
        esi = -1;
        ...
        if (closed) {
            ...
            while (to_stop == 0 && ii >= npo / 2) {
                if (distance(xo[ii], yo[ii], xo[ii - 1],
                        yo[ii - 1]) < PRECISION)
                    new_npo--;
                else
                    to_stop = 1;
        ...
        /*
         * Calculate the angle between two line segments: FCP -> SCP (second
         * clicked point) and OL[FCP] -> OL[FCP+1]
         */
        st = Math.max(opi, osi);
        se = st + 1;
        ...
        /* 
         * Check array size to avoid out of bound error.
         * Check st & se are non negative number.
         */
        int maxIndx = Math.max(st,se);
        if (st >= 0 && se >= 0 && xo.length > maxIndx && yo.length > maxIndx) {
            org_angle = cvm_angl(xo[st], yo[st], xo[se], yo[se]);
        } else {
            org_angle = cvm_angl(xo[0], yo[0], xo[0], yo[0]);
        }
        if (xc.length > 1 && yc.length > 1) {
            new_angle = cvm_angl(xc[0], yc[0], xc[1], yc[1]);
        } else {
            new_angle = cvm_angl(xc[0], yc[0], xc[0], yc[0]);
        }
     ...
     ***********************************************************************/
    {
        ...
        /*
         * If there are limited points on input line or the adjusted starting &
         * end indices overlap, draw whole line as ghost line. Otherwise, ghost
         * line starts at 'is' and goes to 'ie' if is < ie, or starts from 'is'
         * and passes the end of line (also the beginning) and then goes to 'ie'
         * when is > ie.
         */
    ...
    private double[][] cvm_opmd(double[] xo, double[] yo, double[] xc,
            double[] yc, int fin, int fis, int lin, int lis, double drct)
    /************************************************************************
     *cvm_opmd                                                              *
     *                                                                      *
     ...
     ***********************************************************************/
    {
        ...
        // direction FCP->SCP 0 - same as OL -1 - opposite
        FCP_d = 0;
        // direction FCP->LCP 0 - same as OL -1 - opposite
        F_L = 0;
         ...
         */
        if (LCP >= 0 && ((FCP < LCP && FCP_d == 0) || (FCP > LCP && FCP_d == -1)
            ...
            // Starting index for modification
            is = isp;

            // Ending index for modification
            ie = isp + 1 + npc;
     ...
     ***********************************************************************/
    {
        ...
        if (indx < (npo - 1)) {
            ...
            // Check to avoid out of bound error
            if (xo.length > 1 && yo.length > 1) {
                alpha = cvm_angl(xo[indx - 1], yo[indx - 1], xo[indx],
                        yo[indx]);
            } else {
                alpha = cvm_angl(xo[0], yo[0], xo[0], yo[0]);
            }
        }
        ...
        if (indx > 0) {
            ...
            if (xo.length > 1 && yo.length > 1) {
                ...
                beta = cvm_angl(xo[0], yo[0], xo[0], yo[0]);
            }
        }
    ...
    private double[][] cvm_csmd(double[] xo, double[] yo, double[] xck,
            double[] yck, int fin, int fis, int lin, int lis, double drct)
    /************************************************************************
     * cvm_csmd                                                             *
     *                                                                      *
     ...
     ***********************************************************************/
    {
        ...
        // OL[LCP] -> CL[LCP], when LCP off line
        LCP_drct = 0;
        ...
        /*
         * Calculate the starting modify direction (FCP_drct), the ending modify
         * direction (LCP_drct, if LCP offline) and the direction from segment
         * FCP->LCP relative to OL.
         */
        if (fis >= 0) {
            // FCP online
            if (drct > 90. && drct < 270.)
        ...
        if (FCP == LCP) {
            ...
            if (fis >= 0 && lis >= 0 && ((FCP_to_LCP >= 0 && FCP_drct >= 0)
                    || (FCP_to_LCP < 0 && FCP_drct < 0))) {
         ...
         */
        if (FCP_drct >= 0) {
            // CL runs same direction of original
            isp = FCP;
            iep = LCP;
            if (lis >= 0)
                iep = LCP + 1;
        } else {
            // CL runs opposite direction of original
            isp = LCP;
            iep = FCP;
            if (fis >= 0 && (lin >= 0 || lis >= 0))
                iep = FCP + 1;
        ...
        if ((FCP_drct >= 0 && FCP < LCP) || (FCP_drct < 0 && FCP > LCP)) {
             ...
             * Ending segment
             */
            if (distance(xc[npc - 1], yc[npc - 1], xo[iep],
                    yo[iep]) <= TIE_DIST) {
                iep++;
            }
