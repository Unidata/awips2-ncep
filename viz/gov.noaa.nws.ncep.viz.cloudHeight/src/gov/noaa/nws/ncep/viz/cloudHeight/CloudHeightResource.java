 ...
 * 09/20/2016   R15716      S. Russell  Updated paintInternal(), replaced
 *                                      deprecated target.drawline() with
 *                                      IWireframeShape.  Also removed 
 *                                      incomplete and malfunctioning nearest
 *                                      station location feature.
 * </pre>
 * 
 * @version 1
 */
public class CloudHeightResource extends
        AbstractVizResource<CloudHeightResourceData, NCMapDescriptor> {
    ...
    // Suppress, Eclipse warning is wrong
    @SuppressWarnings("unused")
    private Coordinate nearestLoc = null;

    private StationData[] stnLocs = null;

    // list of times normalized by range&offset
    private List<DataTime> stnTimes;
    ...
    public class StationData {
        ...
        StationData(String stationId, Coordinate stationCoordinate,
                DataTime stationRefTime2) {
    ...
    public CloudHeightResource(CloudHeightResourceData resourceData,
        ...
        if (NcEditorUtil.getNcDisplayType(mapEditor) != NcDisplayType.NMAP_DISPLAY) {
            throw new VizException(
                    "Can not use Cloud Height on non-map display type");
        }

        name = "Cloud Height";
        stationDataMap = new HashMap<>(0);
        minimumDistance = INVALID_DISTANCE;
        this.stnTimes = new ArrayList<>(0);
    ...
    public void paintInternal(IGraphicsTarget target, PaintProperties paintProps)
        ...
        if (seldLoc != null) {
            ...
            double[] p1 = getDescriptor().worldToPixel(
                    new double[] { seldLoc.x, seldLoc.y });
            if (p1 != null) {
                IWireframeShape wfsCrossHair = target.createWireframeShape(
                        false, descriptor);

                double[][] line1 = new double[][] { { p1[0] + d, p1[1] },
                        { p1[0] - d, p1[1] } };

                double[][] line2 = new double[][] { { p1[0], p1[1] + d },
                        { p1[0], p1[1] - d } };

                wfsCrossHair.addLineSegment(line1);
                wfsCrossHair.addLineSegment(line2);
                wfsCrossHair.compile();
                target.drawWireframeShape(wfsCrossHair, color,
                        getCapability(OutlineCapability.class)
                                .getOutlineWidth());
                wfsCrossHair.dispose();
    ...
    public StationData getStationData(Coordinate loc, Double maxDist,
            DataTime currFrameTime, int maxIntervalInHoursForValidStationData) {
        return getNearestStation(loc, maxDist, currFrameTime,
    ...
    private void getStationDataFromDataBase() {
        ...
        // TODO: pass the table name as one of the arguments to this method, if
        // the station information is to be accessed
        // from other tables like the BUFRUA. Or pass a different query string
        // altogether, if the field names vary as well.
        queryString
                .append("select reftime, latitude, longitude, stationId from ");
        queryString.append(UPPER_AIR_TABLE);
        queryString.append(" where nil=FALSE ;");
        try {
            /* execute the query */
            List<Object[]> results = new ArrayList<>(
                    DirectDbQuery.executeQuery(queryString.toString(),
            ...
            if (results != null && !results.isEmpty()) {
                for (Object[] eachObjectArray : results) {
                    if (eachObjectArray.length == 4) {
                        ...
                        // Sanity check to avoid creating a StationData object
                        // with invalid coordinates

                        if ((latitude.doubleValue() >= -90.0 && latitude
                                .doubleValue() <= 90.0)
                                && (longitude.doubleValue() >= -180.0 && longitude
                                        .doubleValue() <= 180.0)) {
                            Coordinate stationCoordinate = new Coordinate(
                                    longitude.doubleValue(),
                                    latitude.doubleValue());
                            String stationId = (String) (eachObjectArray[3]);
                            StationData stationData = new StationData(
                                    stationId, stationCoordinate, stationTime);
                            List<StationData> aListOfStationData = new ArrayList<>(
                                    0);
                            if (!this.stationDataMap.isEmpty()
                                    && this.stationDataMap
                                            .containsKey(stationTime)) {
                                aListOfStationData = this.stationDataMap
                                        .get(stationTime);
                            }
                            aListOfStationData.add(stationData);
                            this.stationDataMap.put(stationTime,
                                    aListOfStationData);
     ...
     * 
     * @param loc
     *            - the coordinate of the point clicked by the user
     * @param soundingDist
     *            - the maximum distance from the user's selected point
     * @param currFrameTime
     *            - the satellite image time
     * @param maxIntervalInHoursForValidStationData
     *            - the maximum interval (in hours) on either side of the
     *            satellite image time
     * @return the station closest to the point clicked by the user.
     */
    protected StationData getNearestStation(Coordinate loc,
            Double soundingDist, DataTime currFrameTime,
            int maxIntervalInHoursForValidStationData) {
     ...
     * @return
     */
    private List<StationData> findListOfTimeMatchedStations(DataTime imageTime,
            int maxIntervalInHoursForValidStationData) {
        List<StationData> listOfStationData = new ArrayList<>(0);
        int maxIntervalForStationDataInMinutes = maxIntervalInHoursForValidStationData
                * NUM_MINS_IN_ONE_HOUR;
        int baseOffset = 0;

        while (((baseOffset >= -maxIntervalForStationDataInMinutes) && (baseOffset <= maxIntervalForStationDataInMinutes))) {
            listOfStationData.addAll(new ArrayList<>(
                    getListOfStationDataClosestToFrameImageTime(imageTime,
                            baseOffset)));
     ...
     * @return
     */
    private List<StationData> getListOfStationDataClosestToFrameImageTime(
            DataTime imageTime, int offset) {
        ...
        for (DataTime eachStationTime : setOfDataTimes) {
            if (isTimeMatchedListOfStationsFound(new DataTime(timeToCompare),
                    eachStationTime)) {
                ...
                if (isTimeMatchedListOfStationsFound(new DataTime(
                        timeToCompareAgain), eachStationTime)) {
                    listOfStationData = this.stationDataMap
                            .get(eachStationTime);
                }
            }
        }
    ...
    private boolean isTimeMatchedListOfStationsFound(DataTime imageTime,
            DataTime stationTime) {
        boolean isFound = false;
        long imageTimeInMillis = imageTime.getRefTimeAsCalendar()
                .getTimeInMillis();
        long stationTimeInMillis = stationTime.getRefTimeAsCalendar()
                .getTimeInMillis();
        long numMilliSeconds = (Math.abs(imageTimeInMillis
                - stationTimeInMillis));
