...
public class ChangeLatLonAttributesDialog extends
    ...
    private final LineStyle[] lineStyleButtonSequence = { LineStyle.DOTS,
     ...
     * @param dialogTitle
     */
    public ChangeLatLonAttributesDialog(Shell parentShell,
    ...
    public Composite createDialog(Composite composite) {
        ...
        if (lineStyle.getAttrClass() != LineStyle.class) {
            ...
            statusHandler.error("latInterval is not of expected class? "
                    + latInterval.getAttrClass().toString());
        } else if (lonInterval.getAttrClass() != Integer.class) {
            statusHandler.error("lonInterval is not of expected class? "
                    + lonInterval.getAttrClass().toString());
        }
        ...
        Group selectLatLonIncrementIntrvalGroup = new Group(composite,
                SWT.SHADOW_NONE);
        ...
        selectLatLonIncrementIntrvalGroup
                .setLayout(latLonIncrementIntrvalGridLayout);

        FormData latLonIncrementIntrvalFormData = new FormData();
        latLonIncrementIntrvalFormData.top = new FormAttachment(
                selectLineStyleGroup, 6);
        latLonIncrementIntrvalFormData.left = new FormAttachment(2, 0);
        selectLatLonIncrementIntrvalGroup
                .setLayoutData(latLonIncrementIntrvalFormData);

        Composite intervalLatLonButtonComposite = new Composite(
                selectLatLonIncrementIntrvalGroup, SWT.SHADOW_NONE);
        GridLayout latLonIncrementIntrvalButtonGridLayout = new GridLayout();
        latLonIncrementIntrvalButtonGridLayout.numColumns = 8;
        intervalLatLonButtonComposite
                .setLayout(latLonIncrementIntrvalButtonGridLayout);

        Composite intervalLatLonTextComposite = new Composite(
                selectLatLonIncrementIntrvalGroup, SWT.SHADOW_NONE);
        GridLayout latLonIncrementIntrvalTextGridLayout = new GridLayout();
        latLonIncrementIntrvalTextGridLayout.numColumns = 4;
        intervalLatLonTextComposite
                .setLayout(latLonIncrementIntrvalTextGridLayout);
        ...
        // ...for 2-column grid layout
        final int[] lineWidthButtonSequence = { 0, 2, 1, 3 };
        ...
        // Line Style
        lineStyleButtonMap = new EnumMap<>(LineStyle.class);
        ...
        lineStyleButtonMap.get(lineStyle.getAttrValue()).setSelection(true);
        ...
        Label latLonIncrementsLabel = new Label(intervalLatLonButtonComposite,
                SWT.NONE);
        ...
        for (int i = 0; i < incrementValueArray.length; i++) {
            Button intervalButton = new Button(intervalLatLonButtonComposite,
                    SWT.PUSH);
            GridData gridData = new GridData();
        ...
        latitudeText.setText(String.valueOf(latInterval.getAttrValue()));
        ...
        // we could add one or more of Verify,Focus,or selection Listeners but
        // the Modify seems to
        // catch it all.

        Label longitudeLabel = new Label(intervalLatLonTextComposite, SWT.NONE
                | SWT.RIGHT_TO_LEFT);
        ...
        longitudeText.setText(String.valueOf(lonInterval.getAttrValue()));
        ...
        for (int i = 0; i < intervalButtonArray.length; i++) {
            intervalButtonArray[i].addSelectionListener(new SelectionAdapter() {
                @Override
                public void widgetSelected(SelectionEvent event) {
                    String intervalStringValue = (String) event.widget
                            .getData();
    ...
    // and if not then reset the widget with the current value of the
    // RscAttrValue
    //
