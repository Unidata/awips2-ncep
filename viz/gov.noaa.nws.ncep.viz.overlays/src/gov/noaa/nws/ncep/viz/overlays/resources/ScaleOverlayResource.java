 ...
 * 
 */
public class ScaleOverlayResource extends
        AbstractVizResource<ScaleOverlayResourceData, IMapDescriptor> implements
    ...
    protected void paintInternal(IGraphicsTarget target,
        ...
        ScaleModel scaleModel = ScaleModel.values()[resourceData
                .getScaleModel()];
        ScalePosition scalePosition = ScalePosition.values()[resourceData
                .getScalePosition()];
        ScaleUnit scaleUnit = ScaleUnit.values()[resourceData.getScaleUnit()];
        ScaleIntervalMode scaleIntervalMode = ScaleIntervalMode.values()[resourceData
                .getScaleIntervalMode()];
        Integer scaleIntervalValue = resourceData.getScaleIntervalValue();
        ScaleLatMode scaleLatMode = ScaleLatMode.values()[resourceData
                .getScaleLatMode()];
        Integer scaleEffectiveLatitudeValue = resourceData
                .getScaleEffectiveLatitudeValue();
        ScaleTextFont scaleTextFont = ScaleTextFont.values()[resourceData
                .getScaleTextFont()];
        ScaleTextSize scaleTextSize = ScaleTextSize.values()[resourceData
                .getScaleTextSize()];
        ScaleTextStyle scaleTextStyle = ScaleTextStyle.values()[resourceData
                .getScaleTextStyle()];
        ...
        if (scaleLatMode == ScaleLatMode.AUTO_AT_BAR) {
            ...
            double[] right = new double[] { center[0] + lineWidth / 2,
                    center[1] };
        ...
        if (scaleLatMode != ScaleLatMode.AUTO_AT_BAR) {
            ...
            target.drawString(font, "True at " + s
                    + (meanLat >= -0.001 ? "N" : "S"), x0 - 0.75 * yOff,
                    yLegend, 0.0, TextStyle.NORMAL, color,
        ...
        if (scaleIntervalMode == ScaleIntervalMode.MANUAL) {
            ...
            for (i = 1; l < 0.45 * length; i++) {
                ...
                target.drawLine(x0 + l, y0 - yOff, 0.0, x0 + l, y0 + yOff, 0.0,
                        color, 1);
                target.drawString(font, s, x0 + l,
                        y0 - yOff * scaleTextSize.getSoftwareSize(), 0.0,
            ...
            target.drawLine(x0 + length, y0 - yOff, 0.0, x0 + length,
                    y0 + yOff, 0.0, color, 1);
