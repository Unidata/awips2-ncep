 ...
 * 10/11/2016     R17032     A. Su       Disallowed some chars in naming SPF Group and SPF Name.
 * 
 * </pre>
 * 
 * @author`
...
public class ManageSpfControl extends Composite {
    ...
    private void createManageRbdsGroup() {
        ...
        modifySpfGroupCombo = new Combo(selSpfComp, SWT.READ_ONLY
                | SWT.DROP_DOWN);
        ...
        modifySpfNameCombo = new Combo(selSpfComp, SWT.READ_ONLY
                | SWT.DROP_DOWN);
        ...
        seldRbdsLviewer = new ListViewer(mngRbdsGrp, SWT.MULTI | SWT.BORDER
        ...
        fd.bottom = new FormAttachment(seldRbdsLviewer.getList(), -130,
                SWT.BOTTOM);
    ...
    private void createViewersAndListeners() {
        ...
        SelectionAdapter actionBtnListener = new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                if (btnWid.getSelection() && currActionBtn != btnWid) {
        ...
        createSpfNameTxt.addModifyListener(new ModifyListener() {
            ...
            public void modifyText(ModifyEvent e) {
                ...
                spfActionBtn.setEnabled(!seldSpfGroup.isEmpty()
                        && !seldSpfName.isEmpty());
            }
        });
        ...
        addRbdsBtn.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                SelectRbdsDialog selRbdsDlg = new SelectRbdsDialog(shell,
                        "Select RBDs", true, true, false);

                if (!selRbdsDlg.open()) {
                    return;
                }

                ArrayList<AbstractRBD<?>> rbdsToAdd = selRbdsDlg
                        .getSelectedRBDs();
        ...
        removeRbdsBtn.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                int seldIndxs[] = seldRbdsLviewer.getList()
                        .getSelectionIndices();
                ...
                for (int i = seldIndxs.length - 1; i >= 0; i--) {
                    // sanity check (since the button should be disabled if not
                    // User Level),
                    //
                    if (currActionBtn == modifyActionBtn) {
                        LocalizationFile lFile = seldRbdsList.get(seldIndxs[i])
                                .getLocalizationFile();
    ...
    private void setCurrentAction(Button currBtn) {
        ...
        currSpfGroupCombo = (createSpfSeld ? createSpfGroupCombo
                : modifySpfGroupCombo);
        currSpfNameWidget = (createSpfSeld ? createSpfNameTxt
                : modifySpfNameCombo);
        ...
        spfActionBtn
                .setText((currActionBtn == modifyActionBtn ? "Save SPF"
        ...
        seldRbdsLviewer.getList().setBackground(
                (deleteSpfSeld ? modifySpfNameCombo.getParent().getBackground()
                        : createSpfNameTxt.getBackground()));

        currSpfGroupCombo.setItems(SpfsManager.getInstance()
                .getAvailSPFGroups());
    ...
    private void setSeldSpfGroup(String spfGroup) {
        ...
        if (currSpfNameWidget instanceof Combo) {
            ...
            ((Combo) currSpfNameWidget).setItems(SpfsManager.getInstance()
                    .getSpfNamesForGroup(seldSpfGroup));
    ...
    private void setSeldSpfName(String spfName) {
        ...
        if (seldSpfName == null || seldSpfName.isEmpty()) {
            if (currActionBtn != createActionBtn) {
                ...
                // if delete is selected and there are no spfs in the selected
                // group
        ...
        try {
            seldRbdsList = SpfsManager.getInstance().getRbdsFromSpf(
                    seldSpfGroup, seldSpfName, false); // don't resolve Latest
                                                       // Cycle Times

            isUserLevel = SpfsManager.getInstance().isUserLevelSpf(
                    seldSpfGroup, seldSpfName);

            if (isUserLevel) {

            }
            LocalizationContext cntxt = SpfsManager.getInstance()
                    .getSpfContext(seldSpfGroup, seldSpfName);
            spfLocationLbl.setText("Localization="
                    + cntxt.getLocalizationLevel().toString() + ":"
                    + cntxt.getContextName());
        } catch (VizException e) {
            System.out
                    .println("Error getting Rbds from SPF: " + e.getMessage());
            seldRbdsList.clear();
        }
        ...
        if (!seldRbdsList.isEmpty()) {
            StructuredSelection rbdsel = new StructuredSelection(
                    seldRbdsList.get(0));
    ...
    private void rbdVierwSelectionChanged(SelectionChangedEvent event) {
        StructuredSelection sel_rbds = (StructuredSelection) event
                .getSelection();
        rscLviewer.viewRbd(sel_rbds.size() == 1 ? (AbstractRBD<?>) sel_rbds
        ...
        if (currActionBtn == createActionBtn) {
            ...
            isUserLevelSpf = SpfsManager.getInstance().isUserLevelSpf(
                    seldSpfGroup, seldSpfName);
        }
        ...
        if (currActionBtn == deleteActionBtn) {
            ...
            spfActionBtn.setEnabled(!sel_rbds.isEmpty()
                    && !seldSpfGroup.isEmpty() && !seldSpfName.isEmpty());
    ...
    private void renameRbd() {
        StructuredSelection rbdsel = (StructuredSelection) seldRbdsLviewer
                .getSelection();
            ...
            if (lFile != null
                ...
                MessageDialog errDlg = new MessageDialog(shell, "Error", null,
                        "You do not have permissions to rename a "
                                + lFile.getContext().getLocalizationLevel()
                                        .toString() + " Level Rbd.",
        ...
        RenameRbdDialog newSpfFileNameDialog = new RenameRbdDialog(shell,
                selRbd);
    ...
    private void saveSpf() {
        ...
        try {
                        ...
                        if (rbd1 != rbd2) {
                            if (rbd1.getRbdName().equals(rbd2.getRbdName())) {
                                throw new VizException(
                                        "There are duplicate RBD Names in the SPF\n"
                                                + "Rename one of the RBDs named "
                                                + rbd1.getRbdName());
                            }
                        }
                    }
            ...
            // separate SPF file. One reason is that simply adding an rbd to a
            // SITE level file
            // will now require that all/most of the SITE level files will need
            // to be overridden
            ...
            SpfsManager.getInstance().saveSpf(seldSpfGroup, seldSpfName,
                    seldRbdsList, saveRefTimeBtn.getSelection(), false);

            MessageBox mb = new MessageBox(shell, SWT.OK);
            mb.setText("SPF Saved");
            mb.setMessage("\nSPF " + seldSpfGroup + File.separator
                    + seldSpfName + " has been saved.");
            mb.open();
        } catch (VizException e) {
            MessageDialog errDlg = new MessageDialog(shell, "Error", null,
                    "Error Saving SPF " + seldSpfGroup + File.separator
                            + seldSpfName + ":\n\n" + e.getMessage(),
                    MessageDialog.ERROR, new String[] { "OK" }, 0);
            errDlg.open();
        }
    ...
    private void createSpf() {
        ...
        if (!manager.validateName(shell, seldSpfGroup, "Creating SPF",
                "SPF Group")) {
            return;
        }

        // If not a valid name, pop up an MessageDialog to alert the user.
        if (!manager.validateName(shell, seldSpfName, "Creating SPF",
                "SPF Name")) {
            return;
        }

        try {
            manager.createSpf(seldSpfGroup, seldSpfName, seldRbdsList,
                    saveRefTimeBtn.getSelection(), false);

            MessageBox mb = new MessageBox(shell, SWT.OK);
            mb.setText("SPF Created");
            mb.setMessage("\nSPF " + seldSpfGroup + File.separator
                    + seldSpfName + " has been created.");
            mb.open();
            ...
            MessageDialog errDlg = new MessageDialog(shell, "Error", null,
                    "Error Creating Spf " + seldSpfGroup + File.separator
                            + seldSpfName + ":\n\n" + e.getMessage(),
                    MessageDialog.ERROR, new String[] { "OK" }, 0);
            errDlg.open();
        }
    ...
    private void deleteSpf() {
        try {
            ...
            MessageDialog errDlg = new MessageDialog(shell, "Info", null,
