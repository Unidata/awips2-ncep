...
public class EditMiscResourceAttrsDialog extends
    ...
    /*
     * can't define rsc as IMiscResource since this constructor is invoked thru
     * reflection and is looking for INatlCntrsResource
     */
    public EditMiscResourceAttrsDialog(Shell parentShell,
            INatlCntrsResourceData rd, Capabilities capabilities, Boolean apply) {
        super(parentShell, rd, capabilities, apply);
        if (!(this.rscData instanceof IMiscResourceData)) {
            statusHandler
                    .warn("EditMiscResourceAttrsDialog is expecting an IMiscResource object");
            return;
        }
        miscRscAttrs = ((IMiscResourceData) this.rscData)
                .getMiscResourceAttrs();
    }
    ...
    public Composite createDialog(Composite topComp) {
        ...
        GridLayout gridLayout = new GridLayout(
                miscRscAttrs.getNumDisplayColumns(), false);
        ...
        for (MiscResourceAttr miscRscAttr : miscRscAttrs.getMiscResourceAttrs()) {
            ...
            if (miscRscAttr.dispColumn > miscRscAttrs.getNumDisplayColumns()) {
                statusHandler.error("Error in miscRscAttrList for attr "
                        + miscRscAttr.getAttrName()
            ...
            if (miscRscAttr.guiElem == EditElement.CHECK_BOX) {
                final RscAttrValue rscAttr = editedRscAttrSet
                        .getRscAttr(miscRscAttr.getAttrName());

                if (rscAttr == null) {
                    statusHandler.error("Resource Attr "
                            + miscRscAttr.getAttrName()
                            + " not found in rscAttrSet");
                    return gridComp;
                } else if (!(rscAttr.getAttrValue() instanceof Boolean)) {
                    statusHandler.error("Resource Attr "
                            + miscRscAttr.getAttrName()
                            + " is not of expected class Boolean");
                    return gridComp;
                }
                ...
                checkBox.setSelection(((Boolean) rscAttr.getAttrValue())
                        .booleanValue());
                checkBox.addSelectionListener(new SelectionAdapter() {
                    @Override
                    public void widgetSelected(SelectionEvent e) {
                        rscAttr.setAttrValue(new Boolean(checkBox
                                .getSelection()));
                    }
                });
                ...
                final ColorButtonSelector colorSel = new ColorButtonSelector(
                        colComp, 50, 20);
                ...
                /*
                 * We could add parameters for this (similar to the spinner) but
                 * since it is now only used by the PGEN resource, just set the
                 * values for it.
                 */
                ...
                if (rscAttr.getAttrValue() instanceof Integer) {
                    spnrWid.setSelection(((Integer) rscAttr.getAttrValue())
                            .intValue());
                } else if (rscAttr.getAttrValue() instanceof Float) {
                    float scale = (float) Math.pow(10, numDigits);
                    spnrWid.setSelection(Math.round(scale
                            * ((Float) rscAttr.getAttrValue()).floatValue()));
                }
                ...
                spnrWid.addSelectionListener(new SelectionAdapter() {
                    ...
                    public void widgetSelected(SelectionEvent e) {
                        if (rscAttr.getAttrValue() instanceof Integer) {
                            rscAttr.setAttrValue(new Integer(spnrWid
                                    .getSelection()));
                        } else if (rscAttr.getAttrValue() instanceof Float) {
                            float scale = (float) Math.pow(10, numDigits);
                            rscAttr.setAttrValue(new Float((spnrWid
                                    .getSelection()) / scale));
                        }
                    }
                });
