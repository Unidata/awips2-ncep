...
public class IntlSigmetResource extends
            ...
            if ((polygonExtent == null) || polygonExtent.isEmpty()
                ...
                if (polygonExtent.contains("OF")) {
                    ...
                    if (sideOfLineArr != null && sideOfLineArr.length > 1) {
                        ...
                        ArrayList<PixelCoordinate> sideOfLinePixCoordlist = generatePixelCoordinateListFromWorldCoordinateList(
                                sideOfLineList);
    ...
    private class IntlSigmetRscDataObj implements IRscDataObject {
            ...
            if (g.isEmpty() == false) {
                ...
                if (checker.needsChecking()) {
                    ...
                    if (handle) {
                        ...
                        if (sections.size() > 0) {
                            /*
                             * if sections.size() > 0, then the geometry is
                             * clipped
                             */
            ...
            if (arrayLatLonPoints != null && arrayLatLonPoints.length > 0) {
                ...
                for (int i = 0; i < arrayLatLonPoints.length; i++) {
                    ...
                    Coordinate eachCoordinate = new Coordinate(
                            latLonPoint.getLongitude(LatLonPoint.INDEGREES),
                            latLonPoint.getLatitude(LatLonPoint.INDEGREES));
         ...
         */
        ArrayList<WeatherHazardType> generateWeatherHazardEnumList(
                String hazard) {
    ...
    public IRscDataObject[] processRecord(Object isigObj) {
        if (!(isigObj instanceof IntlSigmetRecord)) {
            statusHandler.info(
                    "IntlSigmentResource.processRecord: object is not a "
                            + "IntlSigmetRecord: "
