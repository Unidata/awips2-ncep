...
/**
 * A ColorBar for use by Plot resources to specify conditional coloring (assigns
 * colors to defined interval ranges).
 * 
 * <pre>
 * SOFTWARE HISTORY
 ...
 * 11/16/16     R21762      P. Moyer    Added Transparency Tracking
 * 
 * </pre>
 * 
 * @author sgurung
...
public class ConditionalColorBar {
    ...
    private final ArrayList<Float> intervals = new ArrayList<>();

    @XmlElement(name = "Interval_Color")
    @XmlJavaTypeAdapter(RGBColorAdapter.class)
    private final ArrayList<RGB> intervalRGBs = new ArrayList<>();

    @XmlElement(name = "Interval_Alpha")
    private final ArrayList<Integer> intervalAlphas = new ArrayList<>();
    ...
    private static final IUFStatusHandler logger = UFStatus
            .getHandler(ConditionalColorBar.class);

    public ConditionalColorBar() {
    }

    // create from a list of intervals and colors.
    // There should be 1 more interval than colors since this will start with
    // the
    // first interval as the lower end of the first interval.
    // Float.NEGATIVE_INFINITY and
    // POSITIVE_INFINITY can be used to create intervals below/above a given
    // value.
    //
    ...
    public ConditionalColorBar(ConditionalColorBar cbar) {
        ...
        for (int i = 0; i < cbar.getNumIntervals(); i++) {
            addColorBarInterval(cbar.getIntervalMin(i), cbar.getIntervalMax(i),
                    cbar.getRGB(i), cbar.getAlpha(i));
        }
    }
    ...
    public void addColorBarInterval(Float min, Float max, RGB rgb) {
        addColorBarInterval(min, max, rgb, 255);
    }

    public void addColorBarInterval(Float min, Float max, RGB rgb,
            Integer alpha) {
        if (min >= max) {
            logger.error("addColorBarInterval : min >= max, ");
            return;
        }
            ...
            for (int i = 0; newIntIndx == -1 && i < getNumIntervals(); i++) {
                if (min > intervals.get(i) && min <= intervals.get(i + 1)) {
                    ...
                    if (min < intervals.get(newIntIndx)) {
                        ...
                        intervalAlphas.add(newIntIndx, alpha);
                    }
                    ...
                    while (newIntIndx <= getNumIntervals()
                        ...
                        intervalAlphas.remove(newIntIndx);
                    }
    ...
    public boolean isValueInInterval(int c, Float value, Unit<?> units) {
        ...
        if (c == 0 && value.equals(intervals.get(c))) {
            return true;
        } else {
            // TODO : add support for units
            return (value > intervals.get(c) && value <= intervals.get(c + 1));
        }
    }
    ...
    public void setRGB(int c, RGB rgb) {
        ...
        if (display != null) {
            // get the alpha if it exists and use it for the new color
            Integer alpha = getAlpha(c);
            if (alpha == null) {
                alpha = new Integer(255);
            }
            colors.get(c).dispose();
            colors.set(c, new Color(display, rgb, alpha));
        }
    }

    public Integer getAlpha(int c) {
        return (c < getNumIntervals() ? intervalAlphas.get(c) : null);
    }

    public void setAlpha(int c, Integer alpha) {
        if (c < 0 || c >= getNumIntervals()) {
            return;
        }
        intervalAlphas.set(c, alpha);

        if (display != null) {
            // get the rgb if it exists and use it for the new color
            RGB rgb = getRGB(c);
            if (rgb == null) {
    ...
    public void setIntervalMin(int c, Float min) {
        if (c >= getNumIntervals()) {
            ...
            logger.error("Error setting min interval to -Inf.");
            return;
        } else if (c != 0 && min <= getIntervalMin(c - 1)) {
            logger.error("Error setting min interval < prev min.");
            return;
        } else if (min >= getIntervalMax(c)) {
            logger.error("Error setting min interval > max.");
            return;
        } else {
            intervals.set(c, min);
        }
    ...
    public void setIntervalMax(int c, Float max) {
        if (c >= getNumIntervals()) {
            ...
            logger.error("Error setting max interval to -Inf.");
            return;
        } else if (max <= getIntervalMin(c)) {
            logger.error("Error setting max interval < min.");
            return;
        } else if (c != getNumIntervals() - 1 && max >= getIntervalMax(c + 1)) {
            logger.error("Error setting max interval > next max.");
            return;
        } else {
    ...
    public String getLabelString(int i) {
        ...
        if (intVal == Float.NEGATIVE_INFINITY) {
            ...
            String lblStr = Float.toString(
                    (float) (Math.round(intervals.get(i) * mult) / mult));
            // really should use a Formatter here but since i'm not, clean this
            // up for the case numDecimals = 0 and
    ...
    public void createNewInterval(int c) {
        ...
        // now split the new interval. If the min or max is Inf then we will use
        // the
        ...
        if (intervals.get(c) == Float.NEGATIVE_INFINITY
            ...
            if (intervals.get(c + 1) - intervals.get(c) < 2 * 1 / mult) {
                logger.error("Interval is too small to split");
                return;
            }
    ...
    public float getDiscreteRange() {
        ...
        Float rangeMax = (getIntervalMax(
                numIntrvls - 1) == Float.POSITIVE_INFINITY
                        ? getIntervalMin(numIntrvls - 1)
                        : getIntervalMax(numIntrvls - 1));
    ...
    public int hashCode() {
        ...
        result = prime * result
                + ((intervalAlphas == null) ? 0 : intervalAlphas.hashCode());
        result = prime * result
    ...
    public boolean equals(Object obj) {
        ...
        if (intervalRGBs == null) {
            ...
            return false;
        if (intervalAlphas == null) {
            if (other.intervalAlphas != null)
                return false;
        } else if (!intervalAlphas.equals(other.intervalAlphas))
            return false;
        if (intervals == null) {
    ...
    public Integer getAlphaForInterval(float c) {
