...
/**
 * UI for temporarily editing advanced settings Plot Model Elements of Point
 * Data Resources
 * 
 * 
 * <pre>
...
public class EditPlotModelElementAdvancedDialog extends Dialog {
    ...
    public EditPlotModelElementAdvancedDialog(Shell parentShell,
        ...
        for (PlotParameterDefn plotPrmDefn : plotParamDefns.getParameterDefns()) {
            // TODO: See if the following can be simplified -- in terms of what
            // things ARE, instead of what they're NOT
            if (!plotPrmDefn.getPlotMode().equalsIgnoreCase(
                    PlotParameterDefn.PLOT_MODE_TABLE)
                    && !plotPrmDefn.getPlotMode().equalsIgnoreCase(
                            PlotParameterDefn.PLOT_MODE_BARB)
                    && !plotPrmDefn.getPlotMode().equalsIgnoreCase(
                            PlotParameterDefn.PLOT_MODE_ARROW)
                    && !plotPrmDefn.getPlotMode().equalsIgnoreCase(
                            PlotParameterDefn.PLOT_MODE_DIRECTIONAL)
                    && (plotPrmDefn.getPlotUnit() != null && !plotPrmDefn
                            .getPlotUnit().isEmpty())) {
        ...
        // allParamNamesArray = plotParamDefns.getAllParameterNames( false,
        // false );
    ...
    public void createShell(int x, int y) {
        ...
        mainGrp.setText("Advanced Options for "
                + editedPlotModelElement.getParamName());
        ...
        paramNameCombo.setText((editedPlotModelElement
                .getConditionalParameter() != null ? editedPlotModelElement
        ...
        Composite editConditionalcolorComp = new EditConditionalColorbarComposite(
                mainGrp, SWT.NONE, editedPlotModelElement);
        ...
        okBtn.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                if (paramNameCombo.getText() == null
                        || "".equals(paramNameCombo.getText())) {
                    MessageDialog infoDlg = new MessageDialog(shell, "Message",
                            null, "Please select a Conditional Parameter.",
    ...
    public void updateConditionalParameter() {
        editedPlotModelElement
                .setConditionalParameter(paramNameCombo.getText());
    }

    private void clearAdvancedSettings() {

        MessageDialog confirmDlg = new MessageDialog(shell,
                "Confirm Clear Advanced Settings", null,
                "Are you sure you want to delete the advanced settings for plot parameter '"
                        + editedPlotModelElement.getParamName() + "'?\n",
        ...
        editedPlotModelElement.setConditionalParameter(editedPlotModelElement
                .getParamName());
        editedPlotModelElement
                .setConditionalColorBar(new ConditionalColorBar());
        ok = true;
        shell.dispose();
    }
}
