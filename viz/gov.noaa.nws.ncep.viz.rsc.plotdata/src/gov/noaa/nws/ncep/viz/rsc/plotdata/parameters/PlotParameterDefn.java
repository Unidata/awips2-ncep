 ...
 * PlotParameterDefn
 * 
 * This class represents a Plot Parameter Definition (PPD).  Each PPD instance
 * defines some parameter that can be assigned to one of several positions in a
 * Plot Model for point data display, and its associated characteristics.
 * 
 * For example, a single PPD could define parameter TMPC (the name of the GEMPAK 
 * parameter for air temperature in degrees Celsius) and associate it with Java
 * class AirTemperature (a subclass of AbstractMetParameter), and datastore (HDF5)
 * field "temperature".  (PPDs are thus decoder-dependent, since it's up to the
 * decoder to decide which datastore field name to use.)  The PPD also other
 * attributes such as mode of display.
 * 
 * Plot Parameter Definitions are normally read in from XML files subject to 
 * localization.
 * 
 * <pre>
 * 
 ...
 * ------------ ----------  ----------- --------------------------
 * 21 Oct 2009                          Generated by JAXB
 * 27 Aug 2015  R7757       B. Hebbard  Modify isVectorParameter() to include new "directional" plot mode;
 *                                      add string constants for plot modes
 * 17 Dec 2015  R9579       B. Hebbard  Add a few more string constants
 *
 * </pre>
 * 
...
public class PlotParameterDefn implements ISerializableObject {
    ...
    private static final transient IUFStatusHandler statusHandler = UFStatus
            .getHandler(PlotParameterDefn.class);

    @XmlAttribute
    protected String plotParamName; // ex. 'TMPF'

    // For barb and arrow plotModes this will be a list of 2 metParamNames, 1
    // for the magnitude and 1 for the direction. For directional (arrows),
    // it may have 1 or 2 parameters -- always direction, but magnitude is
    // optional.
    @XmlAttribute
    ...
    // TODO: change this to be dfltPlotUnit since we may want to add the ability
    // for the user to select the units in the plotModel
    ...
    // for plot parameters which need to determine their value from an array of
    // values. (Was plotFunctionTable attribute)
    @XmlAttribute
    ...
    // the array index specifies which index to show. (ex. For Metar
    // presentWeather)
    @XmlAttribute
    ...
    /*
     * Plot Parameter Definition String Constants
     */

    // Text mode

    public final static String PLOT_MODE_TEXT = "text";

    // Vector modes

    public final static String PLOT_MODE_ARROW = "arrow";

    public final static String PLOT_MODE_BARB = "barb";

    public final static String PLOT_MODE_DIRECTIONAL = "directional";

    // Symbol modes

    public final static String PLOT_MODE_MARKER = "marker";

    public final static String PLOT_MODE_TABLE = "table";

    // Other specific string constants

    public final static String PLOT_PARAMETER_NAME_PTND = "PTND";

    public final static String PLOT_TRANSFORM_REVERSE = "reverse";
    ...
    public String[] getDeriveParams() {
        ...
        if (paramNames.length == 0) {
            statusHandler.handle(Priority.PROBLEM,
                    "Error parsing the derived parameter names ("
                            + deriveParams + ") for PlotParameter "
                            + plotParamName);
        }
    ...
    public S2N getPrioritySelector() {
        if (prioritySelector == null) {
            if (prioritySelectTable != null) {
                prioritySelector = S2N.readS2NFile(prioritySelectTable);
            }
        }
    ...
    // create from metParamName for plotMode BARB, ARROW, and DIRECTIONAL
    public String[] getMetParamNamesForVectorPlot() {
        ...
        if (isVectorParameter()) {
            ...
            if (vectPrms != null && vectPrms.length > 0) {
    ...
    // TODO? also confirm that there are 2 (or 1, for directional) metParameters
    // given?
    public Boolean isVectorParameter() {
        // @formatter:off
        return (plotMode.equalsIgnoreCase(PLOT_MODE_BARB) ||
                plotMode.equalsIgnoreCase(PLOT_MODE_ARROW) ||
                plotMode.equalsIgnoreCase(PLOT_MODE_DIRECTIONAL));
        // @formatter:on
    }
