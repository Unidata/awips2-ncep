...
public class GiniSatResource extends AbstractSatelliteResource implements
    ...
    public GiniSatResource(SatelliteResourceData data, LoadProperties props) {
        ...
        if (satRscData.getMetadataMap().containsKey("creatingEntity")
                && satRscData.getMetadataMap().containsKey("sectorID")) {

            legendStr = satRscData.getMetadataMap().get("creatingEntity")
                    .getConstraintValue()
                    + " "
                    + satRscData.getMetadataMap().get("sectorID")
                            .getConstraintValue()
                    + " "
    ...
    public boolean isCloudHeightCompatible() {
        RequestConstraint physElemConstraint = satRscData.getMetadataMap().get(
                "physicalElement");
        if (physElemConstraint != null) {
            ...
            if (physElemConstraint.getConstraintType() == ConstraintType.EQUALS
                    && physElemConstraint.getConstraintValue().equals(
                            "Imager 11 micron IR")) {
                return true;
            }
        }
