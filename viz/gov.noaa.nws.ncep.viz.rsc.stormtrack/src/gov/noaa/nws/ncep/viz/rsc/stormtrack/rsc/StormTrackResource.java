...
public class StormTrackResource extends
    ...
    private class FrameData extends AbstractFrameData {
        ...
        public void populateFrame() {
            ...
            try {
                ...
                if (strmTrkRscData.isForecastResource()) {
                    ...
                    // if( strmTrkRscData.getTimeMatchMethod() ==
                    // TimeMatchMethod.EXACT ) {
                ...
                Object[] pdoList = ((Map<String, Object>) response.getResults()
                        .get(0)).values().toArray();

                // This could really be considered an error. If the limit is
                // reached then this probably
                // took too long and the calling resource should be making its
                // own query for data.
                ...
                for (Object pdo : pdoList) {
                    for (IRscDataObject rscDataObj : processRecord(pdo)) {
                        // Note that for the regular (ie non-forecast/legacy
                        // behaviour) version of this stormTrack resource
                        // the storm track records are time matched just on the
                        // refTimes and not the forecast times.
                        ...
                        // else { this can actually happen if the time is equal
                        // to the begin time
                        // System.out.println("stormTrack obj doesn't time match
                        // to the frame which queried it??? " +
        ...
        public boolean updateFrameData(IRscDataObject rscDataObj) {
            if (rscDataObj instanceof DfltRecordRscDataObj) {
                ...
                // System.out.println("updateFrameData with:
        ...
        public void paint(PaintProperties paintProps) throws VizException {
            ...
            // Turns out that even with a whole screenfull of tracks it doesn't
            // cost us much to draw wireframes offscreen
            // even when zoomed in. This just lets us avoid recomputing the
            // wireframes on a pan.
            // But on a zoom (the end of a zoom) we will still need to recompute
            // because the markere sizes are based on the zoom level.
            // if( prevExtent != null &&
            ...
            if (!recreateWireframes) {
                ...
                if (strmTrkRscData.getColorCodeByWindSpeed()) {
                    if (cbarRscPair == null) {
                        ...
                        // set the color bar's visiblity to match that of the
                        // parent resource
                        // by changing the Resource Parameter's isVisible value.

                        boolean parentVisibility = getProperties().isVisible();
                        cbarRscPair.getProperties()
                                .setVisible(parentVisibility);
                    ...
                    // segment
                        ...
                        for (int windSpdIntrvl = 0; windSpdIntrvl < windSpeedClrBar
                            ...
                            for (StormLocation sloc : st.getTrack()) {
                                ...
                                if (windSpeedClrBar.isValueInInterval(
                                ...
                                } else { // if this track points wind speed is
                                         // not for this interval
                                    ...
                                    // else if( isLast ) {
                                    // // bug where date time label may not be
                                    // displayed???
                                    // }
                                }
                            ...
                            if (!trackSegData.stormLocs.isEmpty()) {
                                ...
                                trackSegData.lastSegment = true; // has to be
                                                                 // the last loc
            ...
            for (StormLocation sloc : trackData.stormLocs) {
                ...
                if (trackData.firstSegment
                    ...
                    if (strmTrkRscData.getDrawBeginDateTime()) {
                        ...
                        String endDateS = Integer
                                .toString(endTimeC.get(Calendar.DAY_OF_MONTH));

                        if (endDateS.length() == 1) {
                            endDateS = "0" + endDateS;
                        }

                        String endHourS = Integer
                                .toString(endTimeC.get(Calendar.HOUR_OF_DAY));
    ...
    protected AbstractFrameData createNewFrame(DataTime frameTime,
            int timeInt) {
        /*
         * The Python script will accept the time string only if it is formatted
         * as given below....
         */
        // String frameTimeStr = new String
        // (QUERY_DATE_FORMAT.format(frameTime.getValidTime().getTime()));
    ...
    // private String createQueryScriptForPython(String frameStartTimeStr,
    // String frameEndTimeStr ){
    // StringBuilder query = new StringBuilder();
    // query.append("import StormTrackTrackRequest\n");
    // query.append("req = StormTrackTrackRequest.StormTrackTrackRequest()\n");
    // /*A single quote succeeds the '(' character and precedes the ')'
    // character
