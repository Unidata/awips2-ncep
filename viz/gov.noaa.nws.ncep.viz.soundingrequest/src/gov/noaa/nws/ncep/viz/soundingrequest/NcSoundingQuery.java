...
//	}
	...
	/*
	 * genericSoundingDataQuery()
	 * used to query observed uair sounding, point forecast (pfc) model sounding, and grid model sounding data.
	 * At least one of {long[] refTime (in ms) ; String[] refTimeStr (in "yyyy-MM-DD HH:mm:ss")} is required for all queries
	 * At least one of {long[] rangeTime (in ms) ; String[] rangeTimeStr (in "yyyy-MM-DD HH:mm:ss")} is required for pfc/grid model sounding queries
	 * At least one of {Coordinate[] latLonAry ; String[] stnIdAry} is required for all queries
	 * String sndType is required for all queries. One of [ObsSndType.NCUAIR.toString(), ObsSndType.BUFRUA.toString(),PfcSndType.NAMSND.toString(),PfcSndType.GFSSND.toString(),MdlSndType.ANY.toString()]
	 * NcSoundingLayer.DataType dataType is not used for now. Use NcSoundingLayer.DataType.ALLDATA for now.
	 * boolean merge: true for almost all queries; false for query "raw" data, currently only Nsharp use it for query "raw" observed uair data.
	 * String level: "-1" for query all levels; or one of  { "1000", "925", "850", "700", "500", "400", "300", "250", "200", "150", "100" } for query mandatory level
	 * String modelType: used for grid model query only, ignored for other queries
	 * boolean interpolation: used for grid model query only, ignored for other queries, normally set to true
	 * boolean useNcSoundingLayer2: true for using NcSoundingLayer2 at returned profile; false for using NcSoundingLayer
	 * boolean pwRequired: true for query precipitation
	 * 
	 *  ncp directly call this method to query ncuair/bufrua/pfcnam/pfcgfs/grid 
	 *  d2d call mdlSoundingQueryByLatLon(), soundingQueryByStnId(), and pfcSoundingQueryByLatLon() for grid/bufua/pfc sounding data, these 
	 *  3 methods then call this method.
	 */
	public static NcSoundingCube genericSoundingDataQuery(long[] refTime,long[] rangeTime,String[] refTimeStr,String[] rangeTimeStr,Coordinate[] latLonAry , String[] stnIdAry, String sndType, NcSoundingLayer.DataType dataType, 
			boolean merge, String level, String modelType, boolean interpolation, boolean useNcSoundingLayer2, boolean pwRequired) {
		...
		try {
            ...
            if ((rslts instanceof NcSoundingCube)) {
        		...
        		if(useNcSoundingLayer2){
        			NcSoundingTools.convertNcSoundingLayerToNcSoundingLayer2(cube.getSoundingProfileList());
        		}
...
//			return(genericSoundingDataQuery( reflTimeAry, null, null, null,coordArray,  null, sndType,  dataType,  merge,  level,null,true, false));
//		}
//		else
//			return null;
//
...
//	}
	 ...
	 */
	// D2D use this interface for grid model data query, one ref time and one range start time once
	...
	//D2D use this for pfc query for one ref time and one sounding range time of a same station using lat/lon
	...
	//USED BY pfc and grid model sounding to get time line for an AVAILABLE FILE
	//Actually we querying range start time lines
	public static NcSoundingTimeLines soundingRangeTimeLineQuery (String sndType, String refTimeStr, String mdlType) {
		NcSoundingTimeLines timeLines = null;
		SoundingServiceRequest request = new SoundingServiceRequest();
	...
	//9173 obsoleting following 3 methods
	//RM#8306 new
...
//	}
	...
	public static NcSoundingStnInfoCollection genericSoundingStnInfoQuery (String sndType,
	...
	//9173 obsoleting following  method
	//not used
