 ...
 * ------------- ---------- ---------- -----------------------------------------
 * Sep 28, 2010  317        X. Guo     Initial Creation.
 * Oct 07, 2010             X. Guo     Re-write functions to handle Loop Control
 * Mar 07, 2011  migration  G. Hull    use NCLoopProperties
 * Jun 30, 2011             C. Chen    changed to support all editors
 * Jul 29, 2011  450        G. Hull    use NCPathManager
 * Jan 11, 2012  497        S. Gurung  Initialize speedLevel
...
public class LoopManagementDialog extends CaveJFACEDialog {
    ...
    private final int loopStopGapX = 92;

    private final int labelGap = 20;
    ...
    private void createLoopControls(Composite comp) {
        ...
        loopSpeedScale.setMaximum(
                calculateFrameScaleValue(loopProperties.getNcepMaxFrameTime()));
        ...
        loopSpeedScale.addListener(SWT.Selection, new Listener() {
            ...
            public void handleEvent(Event event) {
                if (isLooping && loopProperties.getMode() == LoopMode.Forward) {
                    loopProperties.setFwdFrameTime(
                            calculateFrameTime(loopSpeedScale));
                } else if (isLooping
                        && loopProperties.getMode() == LoopMode.Backward) {
                    loopProperties.setRevFrameTime(
                            calculateFrameTime(loopSpeedScale));
                } else {
                    loopProperties.setFwdFrameTime(
                            calculateFrameTime(loopSpeedScale));
        ...
        firstFrameDwellScale.setMaximum(calculateFirstFrameDwellScaleValue(
                loopProperties.getNcepMaxFirstFrameTime()));
        ...
        firstFrameDwellScale.addListener(SWT.Selection, new Listener() {
            ...
            public void handleEvent(Event event) {
                loopProperties.setFirstFrameDwell(
                        calculateFirstFrameDwellTime(firstFrameDwellScale));
                speedLevel = NOT_IN_SPEED_LEVEL;
                update();
            }
        });
        ...
        lastFrameDwellScale.setMaximum(calculateLastFrameDwellScaleValue(
                loopProperties.getNcepMaxLastFrameTime()));
        ...
        lastFrameDwellScale.addListener(SWT.Selection, new Listener() {
            ...
            public void handleEvent(Event event) {
                loopProperties.setLastFrameDwell(
                        calculateLastFrameDwellTime(lastFrameDwellScale));
                speedLevel = NOT_IN_SPEED_LEVEL;
                update();
            }
        });
    ...
    private void createLoopStop(Composite comp) {
        ...
        curBtn.setBounds(loopStopGp.getBounds().x + loopStopGapX,
                loopStopGp.getBounds().y + labelGap, labelGap, btnHeight);

        Label curLabel = new Label(loopStopGp, SWT.BOLD);

        curLabel.setBounds(
                loopStopGp.getBounds().x + loopStopGapX + labelGap + 2,
                loopStopGp.getBounds().y + labelGap + 2, labelWidth, btnHeight);
        ...
        endBtn.setBounds(loopStopGp.getBounds().x + loopStopGapX * 3,
                loopStopGp.getBounds().y + labelGap, labelGap, btnHeight);
        if (loopStopCurrent) {
        ...
        endLabel.setBounds(
                loopStopGp.getBounds().x + loopStopGapX * 3 + labelGap + 2,
                loopStopGp.getBounds().y + labelGap + 2, labelWidth + 36,
                btnHeight);
    ...
    private void createLoopLayerLink(Composite comp) {
        ...
        enableLabel.setBounds(
                loopLayerLkGp.getBounds().x + loopStopGapX + labelGap + 2,
                loopLayerLkGp.getBounds().y + labelGap + 2, labelWidth,
                btnHeight);
        ...
        endLabel.setBounds(
                loopLayerLkGp.getBounds().x + loopStopGapX * 3 + labelGap + 2,
                loopLayerLkGp.getBounds().y + labelGap + 2, labelWidth + 36,
                btnHeight);
    ...
    private void readDwellRateTable() {
        if (dwellRateList == null) {
            File dwellRateFile = NcPathManager.getInstance()
                    .getStaticFile(NcPathConstants.LOOP_SPEEDS_TBL);
    ...
    private void initLoopProperty() {
        if (!(editor.getLoopProperties() instanceof NCLoopProperties)) {
            System.out.println(
                    "sanity check: editor LoopProperties is not NCLoopProperties");
        }
    ...
    private void initDialogControls() {
        ...
        if (!initDwellRate && speedLevelNum > 0) {
            ...
            if (speed_buttons != null) {
                speed_buttons[speedLevel].setSelection(true);
            }
        }
    ...
    private void setSpeedLevel() {
        ...
        if (speedLevelNum > 0 && speedLevel >= 0) {
            int fwdFrameTime = (int) (dwellRateList.get(speedLevel)
                    .getMidFrameDwell() * 1000);
            if (fwdFrameTime > loopProperties.getNcepMaxFrameTime()) {
                fwdFrameTime = loopProperties.getNcepMaxFrameTime();
            }

            int firstFrameDwellTime = (int) (dwellRateList.get(speedLevel)
                    .getFirstFrameDwell() * 1000);
            if (firstFrameDwellTime > loopProperties
                    .getNcepMaxFirstFrameTime()) {
                firstFrameDwellTime = loopProperties.getNcepMaxFirstFrameTime();
            }
            loopProperties.setFirstFrameDwell(firstFrameDwellTime);

            int lastFrameDwellTime = (int) (dwellRateList.get(speedLevel)
                    .getLastFrameDwell() * 1000);
    ...
    private void setNcepFrameTime() {
        ...
        // Get dwell rates from the table.
        if (speedLevelNum > 0) {
            frameTime = (int) (dwellRateList.get(0).getMidFrameDwell()
                    * TimeUtil.MILLIS_PER_SECOND);
            firstFrameTime = (int) (dwellRateList.get(0).getFirstFrameDwell()
                    * TimeUtil.MILLIS_PER_SECOND);
            lastFrameTime = (int) (dwellRateList.get(0).getLastFrameDwell()
                    * TimeUtil.MILLIS_PER_SECOND);
        } // use default value
        else {
    ...
    private void update() {
        ...
        firstFrameDwellScale.setSelection(
                calculateFirstFrameDwellScaleValue(firstFrameDwellTime));
        firstFrameDwellLabel.setText(formatDwellTime(firstFrameDwellTime));

        // Update last frame
        int lastFrameDwellTime = loopProperties.getLastFrameDwell();
        lastFrameDwellScale.setSelection(
                calculateLastFrameDwellScaleValue(lastFrameDwellTime));
    ...
    private int calculateFrameTime(Scale scale) {
        return LoopProperties.FRAME_STEP * (scale.getSelection());
    }
    ...
    private int calculateFrameScaleValue(int frameTime) {
        int value = frameTime / LoopProperties.FRAME_STEP;
    ...
    private int calculateFirstFrameDwellScaleValue(int dwellTime) {
        int value = dwellTime / LoopProperties.FRAME_STEP;
        return value;
    }
    ...
    private void initSpeedLevel() {
        for (int i = 0; i < speedLevelNum; i++) {
            if (loopProperties.getFwdFrameTime() >= dwellRateList.get(i)
                    .getMidFrameDwell() * 1000) {
                speedLevel = i;
                break;
            }
