...
/**
 * Methods that used to be in NatlCntrsEditor but did little more than delegate
 * to the PaneManager. This helped in migrating the code that called these
 * methods.
 * 
 * <pre>
 * 
...
public class NcEditorUtil {
    ...
    public static AbstractNcPaneManager getNcPaneManager(AbstractEditor vized) {
        ...
        if (edin instanceof EditorInput && ((EditorInput) edin)
            ...
            /*
             * throw error ?? what should be the behavior if called with a
             * non-NC editor?
             */
            return null;
        }
    ...
    public static NcDisplayName getDisplayName(AbstractEditor vized) {
        AbstractNcPaneManager pm = getNcPaneManager(vized);
        return (pm != null ? pm.getDisplayName()
                : new NcDisplayName(0, "error"));
    }
    ...
    public static void setGeoSyncPanesEnabled(AbstractEditor vized,
            Boolean gsync) {
        AbstractNcPaneManager pm = getNcPaneManager(vized);
        if (pm != null) {
    ...
    public static IDescriptor getDescriptor(AbstractEditor vized) {
    ...
    public static void addSelectedPaneChangedListener(AbstractEditor vized,
    ...
    public static void removeSelectedPaneChangedListener(AbstractEditor vized,
            ISelectedPanesChangedListener listener) {
        AbstractNcPaneManager pm = getNcPaneManager(vized);
        if (pm != null) {
    ...
    public static IDisplayPane[] getSelectedPanes(AbstractEditor vized) {
        AbstractNcPaneManager pm = getNcPaneManager(vized);
        return (pm != null
                ? pm.getSelectedPanes(NCPaneManager.NC_PANE_SELECT_ACTION)
                : null);
    }

    public static Boolean isSelectedPane(AbstractEditor vized,
            IDisplayPane pane) {
        AbstractNcPaneManager pm = getNcPaneManager(vized);
        return (pm != null
                ? pm.isSelectedPane(NCPaneManager.NC_PANE_SELECT_ACTION, pane)
                : false);
    }
    ...
    public static void refreshGUIElements(AbstractEditor vized) {
        ...
        ICommandService service = (ICommandService) vized.getSite()
                .getService(ICommandService.class);

        /*
         * wanted to put this in Common but this created cyclic dependencies on
         * the projects.
         */
        ...
        /*
         * the IVizEditorChangedListener will trigger on a close editor which
         * can happen on exit. In this case there is no active page
         */
        ...
        if (actPage != null && actPage.getActiveEditor() != null) {
            ...
            SubStatusLineManager bar = (SubStatusLineManager) actPage
    ...
    /*
     * this is called directly when a pane is selected (ie. not through a
     * PaneChangedListener, although it could be). It is meant to refresh any
     * GUI elements that are based on values from the selected pane(s). To begin
     * with it is only used for the Zoom Lock(Suspend) which will disable the
     * Zoom/UnZoom buttons by setting the state in the ZoomStateSourceProvider.
     */

    public static void refreshGUIElementsForSelectedPanes(
            AbstractEditor vized) {
        AbstractNcPaneManager pm = getNcPaneManager(vized);
        if (pm == null) {
            return;
        }
        ...
        for (IDisplayPane pane : getSelectedPanes(vized)) {
            if (((INatlCntrsDescriptor) pane.getDescriptor())
                    .getSuspendZoom()) {
                lockZoom = true;
            }
        }
        ...
        ISourceProviderService spService = (ISourceProviderService) ww
    ...
    //
    public static IDisplayPane getDisplayPane(AbstractEditor vized,
            INcPaneID pid) {
    ...
    public static void selectPane(AbstractEditor vized, IDisplayPane selPane,
            boolean radioBehaviour) {
        ...
        if (radioBehaviour) {
            seldPanes = new ArrayList<>();
            seldPanes.add(selPane);
        } else {
            IDisplayPane[] dispPanes = getSelectedPanes(vized); // getDisplayPanes();
            seldPanes = new ArrayList<>(Arrays.asList(dispPanes));

            if (seldPanes.contains(selPane) && seldPanes.size() > 1) {
    ...
    /*
     * Currently this just sets the listener for 1 (the active) pane. This works
     * now because all panes have the same timeline and this is used just for
     * the FramdeDataDisplay. TODO : if each pane will need to be notified for a
     * frameChange then we will need to change this to add the listener to all
     * descriptors.
     */

    public static void addFrameChangedListener(AbstractEditor vized,
            IFrameChangedListener lstnr) {
        vized.getActiveDisplayPane().getDescriptor()
                .addFrameChangedListener(lstnr);
    }

    // remove from all descriptors
    public static void removeFrameChangedListener(AbstractEditor vized,
            IFrameChangedListener lstnr) {
