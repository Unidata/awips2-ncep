 ...
 * Nov 29, 2018 #7563      dgilling     Remove unnecessary checked exceptions.
 *
 * </pre>
 *
 * @author gzhang
...
public class SeekInfo {
     ...
     */
    private static final Map<String, String[]> TYPE_DB_TABLE_MAP = new LinkedHashMap<>();
    ...
    private static Map<Integer, QueryResultRow[]> QUERY_RESULTS_MAP = new HashMap<>();
    ...
    static {
        try {
            ...
            System.out.println(
                    "_______SeekInfo: static initialization block: Error: "
                            + e.getMessage());
        }

    }

    private static class LocatorPoitntDataComparator
            implements Comparator<SeekPointData> {
    ...
    /**
     * Read the seekStns.xml file and store info in data structures the
     * attribute names must match those in seekStns.xml.
     *
     * The seek GUI is relatively stable thus the query fields; but the station
     * types may change, a new station type in the database may have different
     * fields names; e.x. station_id vs. stid, latitude vs. lat. We use the xml
     * file attributes to represent them so query can be write without change.
     */
    private static void parseSeekStnsFile() {
        ...
        try {
            ...
            File file = NcPathManager.getInstance()
                    .getStaticFile(NcPathConstants.SEEK_STN_TBL);
            DocumentBuilder builder = factory.newDocumentBuilder();
            doc = builder.parse(file.getAbsoluteFile());

        } catch (Exception e) {
            System.out.println("________SeekInfo: parseSeekStnsFile(): Error:"
                    + e.getMessage());
        }

        NodeList nlist = doc.getElementsByTagNameNS("*", "*");

        String type = "", db = "", table = "", idfield = "", namefield = "",
                latfield = "", lonfield = "", statefield = "";
        ...
        for (int i = 0; i < nlist.getLength(); i++) {
            ...
            for (int j = 0; j < nnMap.getLength(); j++) {
                ...
                if ("type".equalsIgnoreCase(nAttr.getNodeName().trim())) {
                    ...
                    db = nAttr.getNodeValue();
                        ...
                        .equalsIgnoreCase(nAttr.getNodeName().trim())) {
            ...
            if (!type.isEmpty()) {
                TYPE_DB_TABLE_MAP.put(type, new String[] { db, table, idfield,
                        namefield, latfield, lonfield, statefield });
            }
     ...
     * @param stnType:
     *            the station name
     * @return: the array containing database and table names: idx 0: db; idx 1:
     *          table
     */
    ...
    /**
     * Query method for getting data: Tables have different column names:
     * station_id/stid, latitude/lat, etc and stns.island table has no id at
     * all.
     *
     * Query Strings need to reflect this.
     *
     * @param coord:
     *            the coordinate of the mouse click point;
     * @param stnType:
     *            the station type: vor, city, etc;
     ...
     * @param stnType:
     *            station type.
     * @return: a list of the LocatorPointData relative to the point and
     *          station.
     * @throws DataAccessLayerException
     */
    public static List<SeekPointData> nameIdQuery(double lat, double lon,
            String stnType) {
        ...
        double corr = 0.2;
        List<SeekPointData> pointList = new ArrayList<>();
        ...
        if (isWhereInQuery(dbtbl)) {
            ...
            query1.append(" where ").append(la).append(" between ")
                    .append(minLat).append(" and ").append(maxLat);
            query1.append(" and ").append(lo).append(" between ").append(minLon)
                    .append(" and ").append(maxLon);

            query2.append(" where ").append(la).append(" between ")
                    .append(minLat).append(" and ").append(maxLat);
        }
        ...
        for (QueryResultRow rows : citiesResult) {
            ...
            String n = isIsland ? "" + WORD_SEPERATOR + rows.getColumn(0)
                    : rows.getColumn(0) + WORD_SEPERATOR + rows.getColumn(1);
            pointData.setName(n);
     ...
     *
     * @param query1:
     *            query string within a smaller area;
     * @param query2:
     *            query string within a bigger area.
     * @return: QueryResultRow array.
     */
    private static QueryResultRow[] getcitiesList(String query1,
            String query2) {
        ...
        try {
            ...
            citiesList = NcDirectDbQuery.executeMappedQuery(query1, "ncep",
                    QueryLanguage.SQL);
            if (citiesList != null) {
            ...
            if (citiesResult != null && citiesResult.length >= MAX_POINT_NUM) {
                ...
                citiesList = NcDirectDbQuery.executeMappedQuery(query2, "ncep",
                        QueryLanguage.SQL);
                citiesResult = citiesList.getRows();
            }
        } catch (Exception e) {
            System.out.println("___________SeekInfo: getcitiesList(): Error: "
                    + e.getMessage());
        }
    ...
    /**
     * copied from PointDataResource.java to replace the getPtsData(,,) Find the
     * closest points from given point
     *
     * @param ll:
     ...
     */
    public static SeekPointData[] getClosestPoints(Coordinate ll,
            String stnType) {
        /*
         * Check for invalid Coordinate
         */
        if (ll == null || ll.x > 180.0 || ll.x < -180.0 || ll.y > 90.0
                || ll.y < -90.0) {
            return null;
        }
         ...
         */
        List<SeekPointData> list = new ArrayList<>();

        try {
            list = nameIdQuery(ll.y, ll.x, stnType);
        } catch (Exception e) {
            System.out
                    .println("__________ SeekInfo: getClosestPoints(): Error: "
                            + e.getMessage());
        }
        ...
        for (int ii = 0; ii < list.size(); ii++) {
            ...
            gc.setDestinationGeographicPoint(list.get(ii).getLon(),
                    list.get(ii).getLat());
            double dist = -999.99;
        ...
        int number = (ptDataOut.length > MAX_POINT_NUM) ? MAX_POINT_NUM
                : ptDataOut.length;
    ...
    public static boolean isWhereInQuery(String dbtbl) {
        // TODO: may have other station types
        return "stns.cities".equalsIgnoreCase(dbtbl);
    }
     ...
     *
     * @param name:
     *            String containing station_id/stid + WORD_SEPERATOR + name
     * @param stnType:
     *            type of stations: vors, county, cities, etc
     * @param isList:
     *            used in SWT List or SWT Text
     * @return
     */
    public static String getNameOrId(String name, String stnType,
            boolean isList) {
        if (name == null) {
            return "";
        }
        ...
        if (isList) {
            ...
            if ("VOLCANO".equalsIgnoreCase(stnType)
                ...
                return name.split(SeekInfo.WORD_SEPERATOR)[0].toUpperCase()
                        .trim();
            }
    ...
    private static class PointDataNameComparator
            implements Comparator<SeekPointData> {
    ...
    /**
     * Used for the Text fields typed-in searching in SeekResultsDialog. Based
     * on PointDataResource class' method with the same name.
     *
     ...
     */
    public static SeekPointData[] getMatchedPoints(String prefix,
            String stnType, int comboNo) {
        ...
        for (int i = 0; i < citiesResult.length; i++) {
            ...
            String name = (String) citiesResult[i]
                    .getColumn(isNameOrIdCol(stnType));
            if (name.length() < 2 || name.length() < len) {
                continue;
            }
            ...
            if (name.substring(0, len).equalsIgnoreCase(prefix)) {
                ...
                point.setName(name);
                point.setStateID((state == null || state.isEmpty()) ? ""
                        : (String) citiesResult[i].getColumn(4));

                point.setLat(
                        (Double) citiesResult[i].getColumn(isIsland ? 1 : 2));
                point.setLon(
                        (Double) citiesResult[i].getColumn(isIsland ? 2 : 3));
     ...
     *
     * This method pre-query the database for better performance in typed-in
     * station seek.
     *
     * @param stnType:
     *            the station type;
    ...
    public static void doDBQuery(String stnType, int comboNo) {
        ...
        try {
            ...
            QueryResult citiesList = NcDirectDbQuery.executeMappedQuery(
                    query1.toString(), "ncep", QueryLanguage.SQL);
            if (citiesList != null) {
                QUERY_RESULTS_MAP.put(comboNo, citiesList.getRows()); // queryResults1
                                                                      // =
                                                                      // citiesList.getRows();
            }

        } catch (Exception e) {
            System.out.println("___________ SeekInfo.doDbQuery() Error: "
                    + e.getMessage());
        }
